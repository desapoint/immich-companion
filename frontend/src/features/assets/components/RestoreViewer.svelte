<script lang="ts">
  ...
  import V2MediaViewport from './MediaViewport.svelte';
  ...
  function handleShortcut(event:KeyboardEvent){
    const onInteractiveControl=editableTarget(event.target);
    if(!open||event.defaultPrevented||event.ctrlKey||event.metaKey||event.altKey||onInteractiveControl)return;
    if(isViewerSelectionShortcut(event,onInteractiveControl)&&currentId&&ontoggleselection){event.preventDefault();toggleCurrentSelection();return}
...
<V2ViewerShell {open} title="Trash Viewer" {onclose}>
