...
<section class="relations" class:flat-relations={isAlbum}>
  ...
  {#if busy && !items.length}<p>Loading…</p>{:else if !items.length}<p class="empty">No {title.toLowerCase()} found.</p>{:else}<div class="table-wrap"><table><colgroup><col class="selection-column" /><col /><col class="count-column" /><col class="actions-column" /></colgroup><thead><tr><th class="selection-cell"><input class="styled-checkbox" type="checkbox" aria-label={`Select all visible ${title.toLowerCase()}`} checked={displayRows.length > 0 && displayRows.every((row) => selected.has(row.item.id))} onchange={(event) => togglePageSelection(event.currentTarget.checked)} /></th><th><button class="sort-heading" onclick={() => changeSort('name')} aria-label={`Sort by name${sortLabel('name')}`}>Name <span>{sort === 'name' ? direction === 'asc' ? '↑' : '↓' : '↕'}</span></button></th><th><button class="sort-heading" onclick={() => changeSort('asset_count')} aria-label={`Sort by asset count${sortLabel('asset_count')}`}>Assets <span>{sort === 'asset_count' ? direction === 'asc' ? '↑' : '↓' : '↕'}</span></button></th><th class="actions-heading">Actions</th></tr></thead><tbody>{#each displayRows as row (row.item.id)}{@const item = row.item}<tr><td class="selection-cell"><input class="styled-checkbox" type="checkbox" aria-label={`Select ${item.name}`} checked={selected.has(item.id)} onchange={(event) => { const next = new Set(selected); event.currentTarget.checked ? next.add(item.id) : next.delete(item.id); selected = next; }} /></td><td><div class="tag-name" style={`--depth:${row.depth}`}>
