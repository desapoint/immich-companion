<script lang="ts">
  ...
  import type { AlbumRecord, CollectionDeletePlan } from '../../../lib/types/libraryContracts';
  ...
  async function reconcile(action:string):Promise<void>{if(!await refresh(true)&&collectionRequests.error)throw new Error(collectionRequests.error||`${action} was applied, but albums could not be refreshed.`)}
  ...
  async function confirmDelete(){const plan=pendingPlan;if(!plan||mutating)return;executingDelete=true;try{const result=await libraryData.albums.executeDelete(plan.id);retryDeleteIds=result.results.filter((item)=>item.status==='failed').map((item)=>item.id);const removed=result.results.filter((item)=>item.status!=='failed').map((item)=>item.id);if(removed.length)selection.setMembers(removed,false);retryDeletePlan=result.status==='failed'?result:null;deleteDialogOpen=false;pendingPlan=null;await refresh(true);if(retryDeletePlan)operations.setError(new Error(`${retryDeleteIds.length} album deletion${retryDeleteIds.length===1?'':'s'} failed. The saved plan can be retried.`))}catch(error){operations.setError(error,'Albums could not be deleted.')}finally{executingDelete=false}}
...
{#if deleteDialogOpen&&pendingPlan}<ConfirmDialog title={pendingPlan.status==='failed'?'Retry album deletion?':pendingPlan.targetCount===1?'Delete album?':'Delete albums?'} message={pendingPlan.status==='failed'?`Retry ${pendingPlan.results.filter((item)=>item.status==='failed').length} failed items from the frozen ${pendingPlan.targetCount}-album plan. Completed items will not be repeated.`:pendingPlan.targetCount===1?'This album will be deleted. Assets are not deleted.':`Delete ${pendingPlan.targetCount} selected albums? Assets are not deleted.`} confirmLabel={pendingPlan.status==='failed'?'Retry failed albums':pendingPlan.targetCount===1?'Delete album':`Delete ${pendingPlan.targetCount} albums`} icon="trash" destructive={true} pending={mutating} onconfirm={()=>void confirmDelete()} onclose={()=>{if(!mutating){deleteDialogOpen=false;pendingPlan=null}}}/>{/if}
...
<style>
  ...
  :global([data-page-title="Albums"] .v2-context .v2-section:first-child > .v2-stack > .v2-button[data-variant="primary"]) { width: 100%; }
  ...
  @media (max-width: 620px) {
    .albums-collection-table :global(.v2-collection-description) {
      display: -webkit-box;
      overflow: hidden;
    ...
    .albums-collection-table :global(.v2-table-actions-content) > :global(.v2-button[data-variant='danger']) { grid-column: 1 / -1; }
  }
