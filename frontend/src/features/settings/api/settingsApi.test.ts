...
describe('global sync runtime settings API', () => {
  it('loads and saves persisted global-sync pacing', async () => {
    ...
    await saveSyncRuntimeSettings({
      full_batch_size: 25,
      full_min_batch_delay_seconds: 0.5,
      tag_association_concurrency: 4,
    });
    ...
    expect(fetcher).toHaveBeenNthCalledWith(2, '/api/settings/sync/runtime', expect.objectContaining({
      method: 'PUT', body: '{"full_batch_size":25,"full_min_batch_delay_seconds":0.5,"tag_association_concurrency":4}',
    }));
  });
});
...
describe('duplicate policy API', () => {
  it('loads and saves policy and obtains selectable Immich libraries', async () => {
    const policy = {
      ...
      source_priority: ['immich_uploads', 'unlisted'],
      keeper_tiebreakers: [
        'favorite' as const,
        'resolution' as const,
        'metadata_richness' as const,
        'file_size' as const,
        'oldest_capture' as const,
        'uploaded_at' as const,
      ],
