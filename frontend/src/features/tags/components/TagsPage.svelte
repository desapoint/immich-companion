<script lang="ts">
  ...
  import { visibleSelectionState } from '../../assets/state/collectionSelection';
  ...
  import V2Pagination from '../../../lib/components/ui/CollectionPagination.svelte';
  ...
  import type { CollectionDeletePlan, TagHierarchyRow } from '../../../lib/types/libraryContracts';
  ...
  type TagModal={id:number;mode:'create'|'edit';tagId:string;name:string;color:string|null;parentId:string;parentPath:string;parentOptions:Array<{value:string;label:string;subtitle:string}>};
  ...
  async function saveModal(modal:TagModal){
    ...
    if(modal.mode==='create'){
      const created=await operations.run('Create tag',async()=>{const value=await libraryData.tags.create(modal.name,modal.color,modal.parentId);if(!value)throw new Error('The tag was not created.');return value},{
...
{#each modals as modal (modal.id)}<V2Modal id={`tag-modal-${modal.id}`} title={modal.mode==='create'?'Create tag':`Edit ${modal.name}`} description={modal.mode==='create'?'Create a tag with an optional parent.':'Immich only supports changing a tag’s color. Its name and parent hierarchy are read-only.'} size="md" onclose={()=>{if(!mutating)closeModal(modal.id)}}><V2Stack gap="md"><V2Field label="Name" value={modal.name} disabled={modal.mode==='edit'} onvalueinput={(value)=>updateModal(modal.id,{name:value})}/><ColorField id={`tag-color-${modal.id}`} label="Color" value={modal.color} usedColors={colorsInView} usedColorsLabel="Colors in view" onchange={(color)=>updateModal(modal.id,{color})}/>{#if modal.mode==='create'}<SelectField id={`tag-parent-modal-${modal.id}`} label="Parent" value={modal.parentId} options={modal.parentOptions} allowEmpty={true} searchable={true} searchPlaceholder="Search parent tags or paths…" placeholder="No parent — root tag" onchange={(value)=>updateModal(modal.id,{parentId:value,parentPath:modal.parentOptions.find((option)=>option.value===value)?.label??''})}/><V2Section title="Hierarchy preview"><V2Card><span class="v2-tag-preview"><V2ColorSwatch color={modal.color} size="sm"/><span class="v2-small">{modal.parentPath?`${modal.parentPath} / ${modal.name||'New tag'}`:modal.name||'Root tag'}</span></span></V2Card></V2Section>{:else}<V2Section title="Current hierarchy"><V2Card><V2Stack gap="xs"><span class="v2-tag-preview"><V2ColorSwatch color={modal.color} size="sm"/><span class="v2-small">{modal.parentPath?`${modal.parentPath} / ${modal.name}`:modal.name}</span></span><span class="v2-small v2-muted">Immich does not support moving an existing tag to another parent.</span></V2Stack></V2Card></V2Section>{/if}</V2Stack>{#snippet footer()}<V2Button disabled={mutating} onclick={()=>closeModal(modal.id)}>Cancel</V2Button><V2Button variant="primary" disabled={mutating||(modal.mode==='create'&&!modal.name.trim())} onclick={()=>void saveModal(modal)}>{mutating?(operations.phase==='reconciling'?'Refreshing…':'Saving…'):modal.mode==='create'?'Create tag':'Save color'}</V2Button>{/snippet}</V2Modal>{/each}

<style>
  .v2-tag-preview,.v2-tag-create-pending { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
  :global([data-page-title="Tags"] .v2-context .v2-section:first-child > .v2-stack > .v2-button[data-variant="primary"]) { width: 100%; }
  .v2-mobile-collection-search { display: none; }
  @media (max-width: 880px) {
    .v2-mobile-collection-search { display: grid; gap: 8px; margin: 0 0 12px; padding: 12px; border: 1px solid var(--v2-line); border-radius: 10px; background: var(--v2-surface); }
    .v2-mobile-collection-search > label { color: var(--v2-text); font-size: 12px; font-weight: 800; }
    .v2-mobile-collection-search-controls { display: grid; grid-template-columns: minmax(0, 1fr) auto; gap: 8px; align-items: end; }
    .v2-mobile-collection-search-controls :global(.v2-button) { min-height: 44px; }
    .v2-mobile-collection-search :global(.v2-toggle) { min-height: 44px; }
    .v2-mobile-collection-search :global(.v2-text-block) { margin: 0; }
  }
  @media (max-width: 620px) {
    .tags-collection-table :global(.v2-table-actions-content) > :global(.v2-button[data-variant='danger']) { grid-column: 1 / -1; }
  }
  @media (max-width: 380px) {
    .v2-mobile-collection-search-controls { display: flex; align-items: stretch; }
    .v2-mobile-collection-search-controls > input { min-width: 0; flex: 1 1 auto; }
    .v2-mobile-collection-search-controls :global(.v2-button) { flex: 0 0 auto; }
  }
</style>
