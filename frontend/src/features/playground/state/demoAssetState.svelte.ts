...
export function purgeDemoTrashAssets(ids: readonly string[]): void { const idSet = new Set(ids); demoAssetState.trash_api_items = demoAssetState.trash_api_items.filter((asset) => !idSet.has(asset.id)); for (const id of idSet) delete demoAssetState.trash_relationships[id]; persist(); }
