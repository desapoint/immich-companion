...
import type { DuplicateMemberRecord } from '../types/contracts';
import DuplicateAdmissionEvidence from '../components/DuplicateAdmissionEvidence.svelte';
...
describe('DuplicateAdmissionEvidence', () => {
  it('renders linked admission as a compact intermediate-count pill that opens the admitting asset in a new tab', () => {
    ...
    const { body } = render(DuplicateAdmissionEvidence, {
      props: {
        member: linked,
  ...
  it('does not render a linked pill outside linked validation mode or without a resolvable admitting asset', () => {
    ...
    const nonLinkedMode = render(DuplicateAdmissionEvidence, {
      props: {
        member: linked,
        members: [linked, admittedBy],
        mode: 'reference',
    ...
    const missingParent = render(DuplicateAdmissionEvidence, {
      props: {
        member: linked,
        members: [linked],
        mode: 'linked',
