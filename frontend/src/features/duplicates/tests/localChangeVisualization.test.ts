...
describe('localChangeVisualization', () => {
  ...
  it('aggregates fine Local Changes cells into larger coarse cells by mean changed percentage', () => {
    expect(aggregateLocalChangeGrid([
      [0, 10, 20, 30], [10, 20, 30, 40], [40, 50, 60, 70], [50, 60, 70, 80],
    ], 2, 2)).toEqual({ rows: 2, columns: 2, cells: [[10, 30], [50, 70]] });
  });
  ...
  it('clamps source percentages before coarse aggregation', () => {
    expect(aggregateLocalChangeGrid([[-10, 50], [150, Number.NaN]], 1, 1)?.cells).toEqual([[37.5]]);
  });
  ...
  it('preserves alpha and threshold mappings', () => {
  ...
  it('combines highlight colors and checks label readability', () => {
    ...
    expect(canRenderLocalChangeLabel({ cellWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX + 7, cellHeight: 20, measuredLabelWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX, fontSize: LOCAL_CHANGE_LABEL_FONT_PX })).toBe(false);
    expect(canRenderLocalChangeLabel({ cellWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX + 8, cellHeight: 20, measuredLabelWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX, fontSize: LOCAL_CHANGE_LABEL_FONT_PX })).toBe(true);
    expect(canRenderLocalChangeLabel({ cellWidth: 100, cellHeight: 17, measuredLabelWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX, fontSize: LOCAL_CHANGE_LABEL_FONT_PX })).toBe(false);
    expect(canRenderLocalChangeLabel({ cellWidth: 100, cellHeight: 18, measuredLabelWidth: LOCAL_CHANGE_LABEL_MAX_WIDTH_PX, fontSize: LOCAL_CHANGE_LABEL_FONT_PX })).toBe(true);
  });
});
