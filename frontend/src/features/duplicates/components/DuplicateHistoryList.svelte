<script lang="ts">
  ...
  let { history, mutating, reconciling, onselect, onclear }: { history: DuplicateHistoryRecord[]; mutating: boolean; reconciling: boolean; onselect: (row: DuplicateHistoryRecord) => void; onclear: (row: DuplicateHistoryRecord) => void } = $props();
</script>

<V2Toolbar sticky={false}><V2Badge text="Resolution history" /></V2Toolbar>
<V2Stack gap="sm">{#each history as row (row.id)}<V2Card><div class="v2-history-row"><button class="v2-history-resolution" type="button" disabled={mutating||reconciling} onclick={() => onselect(row)}><span class="v2-history-identity"><b>{row.groupLabel}</b><small>{new Date(row.occurredAt).toLocaleString()}</small></span><span class="v2-history-summary">{row.summary}</span></button><V2Button variant="danger" disabled={mutating||reconciling} onclick={() => onclear(row)}>Clear resolution</V2Button></div></V2Card>{:else}<V2Card><span class="v2-muted">No resolution history in this range.</span></V2Card>{/each}</V2Stack>
