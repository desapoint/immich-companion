<script lang="ts">
  ...
  let decisions=$state<DuplicateDecisionWorkspace>({}),stackWorkspace=$state(createDuplicateStackWorkspace()),selectedGroups=$state<string[]>([]),reviewFilter=$state<DuplicateState|'All groups'|'Auto-ready'|'Selected'>('All groups'),sort=$state('reclaimable:desc');
