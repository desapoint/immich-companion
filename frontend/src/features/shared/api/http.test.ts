...
describe('legacy HTTP client', () => {
  ...
  it('rejects ambiguous body and json input before fetch', async () => {
    const fetchMock = vi.fn();
    vi.stubGlobal('fetch', fetchMock);

    await expect(requestJson('/api/example', {
      method: 'POST',
      body: '{}',
      json: {},
    })).rejects.toThrow('Use either body or json');
