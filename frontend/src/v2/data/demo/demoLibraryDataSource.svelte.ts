...
function resolveAssetTarget(target:AssetSelectionTarget):AssetRecord[]{if(target.kind==='ids'){const ids=new Set(existingAssetIds(target.ids));return(indexedDemoAssets() as AssetRecord[]).filter((asset)=>ids.has(asset.id))}const excluded=new Set(target.excludedIds);return searchAssets(target.criteria).filter((asset)=>!excluded.has(asset.id))}
