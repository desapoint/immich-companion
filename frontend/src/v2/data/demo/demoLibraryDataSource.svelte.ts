  ...
  return {
    ...
    assets: {
      ...
      async setFavorite(ids, favorite) {
        ...
        setDemoAssetsFavorite(affected, favorite);
        return result(affected);
      },
      async setArchived(ids, archived) {
        const affected = existingAssetIds(ids);
        setDemoAssetsArchived(affected, archived);
        return result(affected);
      },
      async sync(ids) {
        const affected = existingAssetIds(ids);
        syncDemoAssets(affected);
        return result(affected);
      },
      async trash(ids) {
        const affected = existingAssetIds(ids);
        trashDemoAssets(affected);
        return result(affected);
      },
      async restore(ids) {
        const affected = existingTrashIds(ids);
        restoreDemoTrashAssets(affected);
        return result(affected);
      },
      async addToAlbum(ids, albumId) {
        const affected = existingAssetIds(ids);
        if (!demoAssetState.albums.some((album) => album.id === albumId)) {
          return result([], affected.map((id) => ({ id, reason: 'Album not found' })));
        }
        addDemoAssetsToAlbum(affected, albumId);
        return result(affected);
      },
      async removeFromAlbums(ids, albumIds) {
        const affected = existingAssetIds(ids);
        removeDemoAssetsFromAlbums(affected, albumIds);
        return result(affected);
      },
      async addTags(ids, tagIds) {
        const affected = existingAssetIds(ids);
        addDemoTagsToAssets(affected, tagIds);
        return result(affected);
      },
      async removeTags(ids, tagIds) {
        const affected = existingAssetIds(ids);
        removeDemoTagsFromAssets(affected, tagIds);
        return result(affected);
      },
      async stack(ids) {
        const affected = existingAssetIds(ids);
        if (affected.length < 2) return result([], affected.map((id) => ({ id, reason: 'At least two assets are required' })));
        stackDemoAssets(affected);
        return result(affected);
      },
      async unstack(ids) {
        const affected = existingAssetIds(ids);
        removeDemoAssetsFromStacks(affected);
        return result(affected);
      },
      async setStackPrimary(assetId) {
        ...
        const affected = [...asset.stack.assets];
        setDemoStackPrimary(assetId);
        return result(affected);
      },
      async removeCompleteStack(assetId) {
        ...
        if (!asset?.stack) return result([], [{ id: assetId, reason: 'Asset is not stacked' }]);
        const affected = [...asset.stack.assets];
        removeDemoCompleteStack(assetId);
        return result(affected);
      },
    },
    albums: {
      ...
      async update(id, patch) {
        if (!demoAssetState.albums.some((album) => album.id === id)) return result([], [{ id, reason: 'Album not found' }]);
        updateDemoAlbum(id, patch);
        return result([id]);
      },
      async delete(ids) {
        const existing = ids.filter((id) => demoAssetState.albums.some((album) => album.id === id));
        deleteDemoAlbums(existing);
    ...
    tags: {
      ...
      async update(id, patch) {
        if (!demoAssetState.tags.some((tag) => tag.id === id)) return result([], [{ id, reason: 'Tag not found' }]);
        updateDemoTag(id, patch);
        return result([id]);
      },
      async delete(ids) {
        const existing = ids.filter((id) => demoAssetState.tags.some((tag) => tag.id === id));
        deleteDemoTags(existing);
        return result(existing);
      },
    },
    media: {
