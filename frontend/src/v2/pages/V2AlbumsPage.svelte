<script lang="ts">
  ...
  let page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),query=$state(''),total=$state(0),albums=$state<AlbumRecord[]>([]),nextCursor=$state<string|null>(null),loading=$state(false),mutating=$state(false),loadError=$state(''),operationError=$state(''),feedback=$state<OperationFeedback|null>(null),retryDeleteIds=$state<string[]>([]),pendingDeleteIds=$state<string[]>([]),deleteDialogOpen=$state(false);
  ...
  async function deleteIds(ids:string[]){if(!ids.length||mutating)return;mutating=true;operationError='';try{const result=await libraryData.albums.delete(ids);feedback=mutationFeedback('Delete albums',result);retryDeleteIds=result.failed.map((failure)=>failure.id);selectedIds=selectedIds.filter((id)=>retryDeleteIds.includes(id));await refresh(true)}catch(error){feedback=null;retryDeleteIds=[];operationError=errorMessage(error,'Albums could not be deleted.')}finally{mutating=false}}
...
<V2PageLayout title="Albums" description="Search, sort, create, edit, delete and use albums to filter the current asset workspace.">
  ...
  <V2Zone>
    ...
    {#if operationError}<V2ErrorState title="Album operation failed" message={operationError}/>{/if}
