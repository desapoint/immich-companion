<script lang="ts">
  ...
  let page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),query=$state(''),total=$state(0),albums=$state<AlbumRecord[]>([]),nextCursor=$state<string|null>(null),loading=$state(false),mutating=$state(false),loadError=$state(''),feedback=$state<OperationFeedback|null>(null),retryDeleteIds=$state<string[]>([]);
  ...
  async function deleteSelected(){await deleteIds([...selectedIds])}
  ...
  async function deleteRow(id:string){await deleteIds([id])}
