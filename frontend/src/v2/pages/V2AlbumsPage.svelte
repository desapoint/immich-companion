<script lang="ts">
  ...
  import V2NoticeDialog from '../components/V2NoticeDialog.svelte';
  ...
  let modalSequence=0,modals=$state<AlbumModal[]>([]),selectedIds=$state<string[]>([]),filterDialogOpen=$state(false);
  ...
  function filterAssets(){filterDialogOpen=true}
...
{#if filterDialogOpen}<V2NoticeDialog id="album-filter-not-implemented" title="Not implemented" message="Filtering assets from an album is not implemented yet." onclose={()=>filterDialogOpen=false}/>{/if}
