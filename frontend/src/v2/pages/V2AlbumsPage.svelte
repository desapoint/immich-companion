...
<V2PageLayout title="Albums" description="Search, sort, create, edit, delete and use albums to filter the Assets workspace.">
  ...
  <V2Zone>
    ...
    <V2Card><V2Table><thead><tr><th></th><th>Name</th><th>Assets</th><th>Description</th><th class="v2-table-actions">Actions</th></tr></thead><tbody>{#each albums as album}<tr><td><input type="checkbox"></td><td><b>{album[0]}</b></td><td>{album[1]}</td><td class="v2-muted">Album description</td><td class="v2-table-actions"><V2Inline class="v2-table-actions-content" gap="sm" justify="end" wrap={false}><V2Button>Filter assets</V2Button><V2Button onclick={() => selected = album[0]}>Edit</V2Button><V2Button variant="danger">Delete</V2Button></V2Inline></td></tr>{/each}</tbody></V2Table></V2Card>
  </V2Zone>
