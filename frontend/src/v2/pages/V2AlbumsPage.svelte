<script lang="ts">
  ...
  function toggleVisible(){if(selectionScope==='All matching'){if(selection.active)selection.clear();else void selection.selectAll({query},visibleIds,total);return}selection.setMembers(visibleIds,visibleSelection!=='all',visibleIds[0]??null)}
  async function requestDelete(ids:string[]=[]){if(mutating)return;if(ids.length){selection.clear();selection.setMembers(ids,true,ids[0]??null)}try{await selection.flush();if(!selection.selectionId)return;pendingPlan=await libraryData.albums.planDelete(selection.selectionId);deleteDialogOpen=true}catch(error){operations.setError(error,'Albums could not be prepared for deletion.')}}
