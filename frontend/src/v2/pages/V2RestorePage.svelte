<script lang="ts">
  ...
  const matchingAssets=$derived.by(()=>{
    const source=[...trashApiDemoAssets()];
  ...
  const total=$derived((demoAssetState.revision,matchingAssets.length));
...
<V2PageLayout title="Restore" description="Restore assets from the shared V2 demo trash while preserving captured relationships.">
