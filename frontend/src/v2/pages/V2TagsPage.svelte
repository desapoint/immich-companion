<script lang="ts">
  ...
  let query=$state(''),lastAppliedQuery=$state(''),includeHierarchy=$state(false),page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),rows=$state<TagHierarchyRow[]>([]),resultTotal=$state(0),nextCursor=$state<string|null>(null),preparingCreate=$state(false),retryDeleteIds=$state<string[]>([]),deleteDialogOpen=$state(false),pendingPlan=$state<CollectionDeletePlan|null>(null),selectionScope=$state<'Current page'|'All matching'>(typeof sessionStorage!=='undefined'&&sessionStorage.getItem('immich-companion:v2:tag-selection-scope')==='All matching'?'All matching':'Current page'),searchSelectionDialog=$state(false);
  ...
  function applySearch(dismiss:boolean){if(dismiss)selection.clear();lastAppliedQuery=query;page=1;searchSelectionDialog=false;void refresh(true)}
  function submitSearch(){if(loading||mutating)return;if(selection.active&&query!==lastAppliedQuery){searchSelectionDialog=true;return}applySearch(false)}
  ...
  function toggleVisible(){if(selectionScope==='All matching'){if(selection.active)selection.clear();else void selection.selectAll({query,includeHierarchy},visibleRealIds,resultTotal);return}selection.setMembers(visibleRealIds,visibleSelection!=='all',visibleRealIds[0]??null)}
  function realTagIdsFor(row:TagHierarchyRow){return row.realTagIds}
  async function requestDelete(ids:string[]=[]){if(mutating)return;if(ids.length){selection.clear();selection.setMembers(ids,true,ids[0]??null)}try{await selection.flush();if(!selection.selectionId)return;pendingPlan=await libraryData.tags.planDelete(selection.selectionId);deleteDialogOpen=true}catch(error){operations.setError(error,'Tags could not be prepared for deletion.')}}
  ...
  async function confirmDelete(){const plan=pendingPlan;if(!plan||mutating)return;try{const result=await libraryData.tags.executeDelete(plan.id);retryDeleteIds=result.results.filter((item)=>item.status==='failed').map((item)=>item.id);deleteDialogOpen=false;pendingPlan=null;await refresh(true)}catch(error){operations.setError(error,'Tags could not be deleted.')}}
