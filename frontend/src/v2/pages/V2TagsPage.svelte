<script lang="ts">
  ...
  let query=$state(''),includeHierarchy=$state(false),selectedIds=$state<string[]>([]),page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),rows=$state<TagHierarchyRow[]>([]),resultTotal=$state(0),nextCursor=$state<string|null>(null),loading=$state(false);
  ...
  async function deleteSelected(){const ids=selectedRealTagIds();if(ids.length)await libraryData.tags.delete(ids);selectedIds=[];await refresh(true)}
