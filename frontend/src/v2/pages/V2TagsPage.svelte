<script lang="ts">
  ...
  let modalSequence=0,modals=$state<TagModal[]>([]),filterDialogOpen=$state(false);
...
{#if filterDialogOpen}<V2NoticeDialog id="tag-filter-not-implemented" title="Not implemented" message="Filtering assets from a tag is not implemented yet." onclose={()=>filterDialogOpen=false}/>{/if}
