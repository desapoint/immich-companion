<script lang="ts">
  ...
  let query=$state(''),appliedQuery=$state(''),includeHierarchy=$state(false),appliedHierarchy=$state(false),page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),rows=$state<TagHierarchyRow[]>([]),resultTotal=$state(0),nextCursor=$state<string|null>(null),preparingCreate=$state(false),retryDeleteIds=$state<string[]>([]),deleteDialogOpen=$state(false),pendingPlan=$state<CollectionDeletePlan|null>(null),selectionScope=$state<'Current page'|'All matching'>(typeof sessionStorage!=='undefined'&&sessionStorage.getItem('immich-companion:v2:tag-selection-scope')==='All matching'?'All matching':'Current page'),searchSelectionDialog=$state(false);
  ...
  function toggleVisible(){if(selectionScope==='All matching'){if(selection.active)selection.clear();else void selection.selectAll({query:appliedQuery,includeHierarchy:appliedHierarchy},visibleRealIds,resultTotal);return}selection.setMembers(visibleRealIds,visibleSelection!=='all',visibleRealIds[0]??null)}
