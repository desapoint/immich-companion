<script lang="ts">
  ...
  const tags=$derived.by(()=>{
    ...
    const leaves:Tag[]=libraryData.state.tags.map((tag)=>{
  ...
  function realTagIdsFor(row:Tag):string[]{if(!row.synthetic)return[row.id];return libraryData.state.tags.filter((tag)=>tag.tag_name.startsWith(`${row.path} / `)).map((tag)=>tag.id)}
  ...
  async function deleteSelected(){const ids=selectedRealTagIds();if(ids.length)await libraryData.tags.delete(ids);selectedIds=[];page=Math.min(page,Math.max(1,Math.ceil(resultTotal/pageSize)))}
  ...
  function parentOptionsFor(modal:TagModal){const editing=libraryData.state.tags.find((tag)=>tag.id===modal.tagId)?.tag_name??'';return tags.filter((tag)=>tag.children>0&&tag.path!==editing&&!tag.path.startsWith(`${editing} / `)).map((tag)=>({value:tag.path,label:tag.name,subtitle:tag.parent||'Root'}))}
  async function saveModal(modal:TagModal){if(!modal.name.trim())return;if(modal.mode==='create')await libraryData.tags.create(modal.name,modal.color||null,modal.parent);else await libraryData.tags.update(modal.tagId,{name:modal.name,color:modal.color||null,parentPath:modal.parent});closeModal(modal.id)}
  async function deleteRow(tag:Tag){await libraryData.tags.delete(realTagIdsFor(tag));selectedIds=selectedIds.filter((id)=>id!==tag.id)}
...
<V2PageLayout title="Tags" description="Search and manage hierarchical tags from the same data source used by Assets filters and relationships.">
...
{#each modals as modal (modal.id)}
  <V2Modal id={`tag-modal-${modal.id}`} title={modal.mode==='create'?'Create tag':`Edit ${modal.name}`} description={modal.mode==='create'?'Create a tag with an optional parent.':'Update this tag and move its descendant branch when its path changes.'} size="md" onclose={()=>closeModal(modal.id)}>
    ...
    {#snippet footer()}<V2Button onclick={()=>closeModal(modal.id)}>Cancel</V2Button><V2Button variant="primary" disabled={!modal.name.trim()} onclick={()=>void saveModal(modal)}>{modal.mode==='create'?'Create tag':'Save changes'}</V2Button>{/snippet}
  </V2Modal>
{/each}
