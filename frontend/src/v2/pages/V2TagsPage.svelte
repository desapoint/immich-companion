<script lang="ts">
  ...
  let query=$state(''),includeHierarchy=$state(false),selectedIds=$state<string[]>([]),page=$state(1),pageSize=$state(24),resultMode=$state<ResultMode>('Pagination'),sort=$state('name:asc'),rows=$state<TagHierarchyRow[]>([]),resultTotal=$state(0),nextCursor=$state<string|null>(null),loading=$state(false),mutating=$state(false),loadError=$state(''),feedback=$state<OperationFeedback|null>(null),retryDeleteIds=$state<string[]>([]);
  ...
  async function deleteSelected(){await deleteIds(selectedRealTagIds())}
  ...
  async function openEdit(tag:TagHierarchyRow){if(tag.synthetic||mutating)return;try{modals=[...modals,{id:++modalSequence,mode:'edit',tagId:tag.id,name:tag.name,color:tag.color??'#9A78FF',parent:tag.parent,parentOptions:await libraryData.tags.parentOptions(tag.id)}]}catch(error){loadError=errorMessage(error,'Parent tag options could not be loaded.')}}
  ...
  async function deleteRow(tag:TagHierarchyRow){await deleteIds(realTagIdsFor(tag))}
...
<V2PageLayout title="Tags" description="Search and manage hierarchical tags through the active data source.">
  ...
  <V2Zone>
    ...
    <V2OperationFeedback {feedback} retryLabel={retryDeleteIds.length?'Retry failed':''} onretry={retryDeleteIds.length?()=>void deleteIds([...retryDeleteIds]):undefined}/>
