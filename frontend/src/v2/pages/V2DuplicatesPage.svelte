<script lang="ts">
  ...
  let decisions=$state<Record<string,DuplicateDecision>>({}),stackWorkspace=$state(createDuplicateStackWorkspace()),selectedGroups=$state<string[]>([]),reviewFilter=$state<DuplicateState|'All groups'|'Auto-ready'>('Actionable'),sort=$state('reclaimable:desc');
  ...
  async function keeperRulesApplied(result:DuplicateKeeperSelectionResult){keeperSummary=`${result.appliedGroupCount.toLocaleString()} groups auto-selected · ${result.trashCount.toLocaleString()} assets marked Delete`;keeperRulesOpen=false;reviewFilter='Actionable';collection.reset();interactionError='';await refreshGroups(true,false)}
