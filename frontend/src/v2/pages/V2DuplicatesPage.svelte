<script lang="ts">
  ...
  async function confirmPendingReview(){const review=pendingReview;if(!review)return;pendingReview=null;if(review.scope==='group'&&review.groupId!==null)await applyGroupDecisionSet(review.groupId,review.resolution);else await applyDecisionSet(review.resolution)}
