<script lang="ts">
  ...
  let tab=$state<DuplicateTab>('Review'),compare=$state(false),group=$state(1),member=$state(0),reference=$state(0),decisions=$state<Record<string,string>>({}),selectedGroups=$state<number[]>([]),reviewFilter=$state('All groups'),sourceIds=$state<string[]>([]),assets=$state<AssetRecord[]>([]);
  ...
  const visibleGroups=$derived(groups.filter((item)=>reviewFilter==='All groups'||item.state===reviewFilter||(reviewFilter==='Auto-ready'&&item.state==='Actionable'))),activeGroup=$derived(groups.find((item)=>item.id===group)),activeAssetIds=$derived(activeGroup?.assetIds??[]),decisionCount=$derived(Object.keys(decisions).length);
  function assetById(id:string){return assets.find((asset)=>asset.id===id)}
  ...
  async function loadSource(){const result=await libraryData.assets.search({mode:'simple',filters:{},sort:{field:'takenDate',direction:'desc'},page:1,pageSize:37});sourceIds=result.items.map((asset)=>asset.id);assets=result.items}
  async function executeDecisions(){const deleteIds=Object.entries(decisions).filter(([,decision])=>decision==='delete').map(([id])=>id),stackIds=new Set(Object.entries(decisions).filter(([,decision])=>decision==='stack').map(([id])=>id));for(const item of groups){const members=item.assetIds.filter((id)=>stackIds.has(id));if(members.length>=2)await libraryData.assets.stack(members)}if(deleteIds.length)await libraryData.assets.trash(deleteIds);decisions={};selectedGroups=[];compare=false;await loadSource()}
  onMount(()=>{void(async()=>{await libraryData.initialize();await loadSource()})()});
</script>

<V2PageLayout title="Duplicates" description="Review duplicate groups using repository-backed asset records.">
