<script lang="ts">
  ...
  let decisions=$state<Record<string,DuplicateDecision>>({}),stackWorkspace=$state(createDuplicateStackWorkspace()),selectedGroups=$state<string[]>([]),reviewFilter=$state<DuplicateState|'All groups'|'Selected'>('Actionable'),sort=$state('reclaimable:desc');
  ...
  async function keeperRulesApplied(result:DuplicateKeeperSelectionResult){keeperSummary=`${result.appliedGroupCount.toLocaleString()} groups auto-selected · ${result.trashCount.toLocaleString()} assets marked Delete`;keeperRulesOpen=false;selectedGroups=[...libraryData.duplicates.selectedGroupIds()];reviewFilter='Selected';collection.reset();interactionError='';await refreshGroups(true,false)}
