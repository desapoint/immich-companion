<script lang="ts">
  ...
  $effect(()=>{
    ...
    if(completed.some((id)=>!selectedGroups.includes(id))){selectedGroups=[...new Set([...selectedGroups,...completed])];persistSelection()}
  });
  ...
  function setDecision(groupId:string,assetId:string,decision:DuplicateDecision){if(!capabilities.decisions.includes(decision))return;decisions={...decisions,[assetId]:decision};stackWorkspace=decision==='stack'?assignAssetToActiveStack(stackWorkspace,groupId,assetId):removeAssetFromPendingStack(stackWorkspace,assetId);const item=groups.find((entry)=>entry.id===groupId);if(item)scheduleDraft(item)}
