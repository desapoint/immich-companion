<script lang="ts">
  ...
  async function runDiscovery(){
    ...
    try{
      ...
      await operations.run('Duplicate discovery',()=>libraryData.duplicates.runDiscovery({similarityThreshold:normalizedThreshold,includeSimilar,includeExact,maxCandidates:normalizedCandidates},(progress)=>backgroundTaskStatus.updateDuplicateDiscovery(progress)),{
...
<V2DuplicateCompareViewer open={compare} groupTitle={activeGroup?duplicateGroupTitle(activeGroup):'Duplicate comparison'} groupKind={activeGroup?.kind??''} groupSimilarity={activeGroup?.groupSimilarity??null} similarityEngine={activeGroup?.similarityEngine??null} similarityModelVersion={activeGroup?.similarityModelVersion??null} similarityFeatureVersion={activeGroup?.similarityFeatureVersion??null} similarityComparisonVersion={activeGroup?.similarityComparisonVersion??null} assetIds={activeAssetIds} similarities={activeSimilarities} decisionOptions={capabilities.decisions} stackLabel={activeCompareStack?.label??'Stack'} stackPrimary={activeCompareStack?.primaryAssetId===activeAssetIds[member]} disabled={mutating} bind:member bind:reference bind:decisions ondecisionchange={(assetId,decision)=>setDecision(group,assetId,decision)} ondecisionclear={(assetId)=>clearDecision(group,assetId)} onstackprimary={setStackPrimary} onreferencechange={switchReference} onclose={()=>{compare=false;persistSelection()}}/>
