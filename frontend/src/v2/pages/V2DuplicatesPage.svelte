...
<V2PageLayout title="Duplicates" description="Review duplicate groups supplied by the active data source, with provider-backed discovery and decisions.">
  ...
  <V2Zone>
    ...
    {#each groups as item}<V2Card class="v2-duplicate-group"><V2Stack gap="md"><V2Inline justify="between" align="start" wrap={true}><V2Inline gap="sm" wrap={true}><input type="checkbox" disabled={mutating} checked={selectedGroups.includes(item.id)} onchange={(event)=>toggleGroup(item.id,event.currentTarget.checked)}><b>Group {item.id}</b><V2Badge text={`${item.members.length} assets`}/><V2Badge text={item.kind}/><V2Badge tone={item.state==='Actionable'?'ok':item.state==='Blocked'?'bad':'warn'} text={item.state}/></V2Inline><V2Inline gap="sm" wrap={true}>{#if capabilities.decisions.includes('keep')}<V2Button disabled={mutating} onclick={()=>presetGroup(item,'keep')}>Keep preset</V2Button>{/if}<V2Button disabled={mutating} onclick={()=>openCompare(item.id,0)}>Compare</V2Button></V2Inline></V2Inline><div class="v2-duplicate-members">
      {#each item.members as member,index}<div class="v2-duplicate-member"><button class="v2-duplicate-image" disabled={mutating} onclick={()=>openCompare(item.id,index)}><V2LazyAssetMedia cacheKey={`duplicate-thumbnail:${member.asset.id}`} resolve={()=>libraryData.media.thumbnail(member.asset)} alt={member.asset.original_file_name}/><span class="v2-duplicate-image-meta"><b>{member.asset.original_file_name}</b><small>{member.asset.library_id?'External library':'Immich'} · {member.similarity.toFixed(1)}%</small></span></button><div class="v2-duplicate-actions">{#each capabilities.decisions as decision}<V2Button disabled={mutating} active={decisions[member.asset.id]===decision} onclick={()=>setDecision(member.asset.id,decision)}>{decision[0].toUpperCase()+decision.slice(1)}</V2Button>{/each}</div></div>{/each}
