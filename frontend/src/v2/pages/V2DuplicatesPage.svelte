<script lang="ts">
  ...
  function pointerDown(e:PointerEvent){if((e.target as HTMLElement).closest('button,input,label'))return;dragging=true;lastX=e.clientX;lastY=e.clientY;(e.currentTarget as HTMLElement).setPointerCapture?.(e.pointerId)}
...
<V2PageLayout title="Duplicates" description="Review exact and similarity duplicate groups, tune discovery rules, and inspect resolution history.">
  ...
  <V2Zone>
    {#if tab==='Review'}
      ...
      {#each groups as g}
        <V2Card class="v2-duplicate-group"><V2Stack gap="md"><V2Inline justify="between" align="start" wrap={true}><V2Inline gap="sm" wrap={true}><input type="checkbox"><b>Group {g.id}</b><V2Badge text={`${g.count} images`}/><V2Badge text={g.kind}/><V2Badge tone={g.tone} text={g.state}/></V2Inline><V2Inline gap="sm" wrap={true}><V2Button>Apply preset</V2Button><V2Button onclick={()=>openCompare(g.id,0)}>Compare</V2Button></V2Inline></V2Inline><div class="v2-duplicate-members">{#each Array.from({length:g.count},(_,i)=>i) as i}<div class="v2-duplicate-member"><button class="v2-duplicate-image" onclick={()=>openCompare(g.id,i)}><img src={demoCompareImage(g.id,i)} alt={`Duplicate member ${i+1}`}><span class="v2-duplicate-image-meta"><b>Member {i+1}</b><small>{i===0?'Suggested keeper':i%3===0?'External':'Immich'}</small></span></button><div class="v2-duplicate-actions"><V2Button variant={i===0?'primary':'default'}>Keep</V2Button><V2Button>Delete</V2Button><V2Button>Stack</V2Button></div></div>{/each}</div></V2Stack></V2Card>
      {/each}
    {:else if tab==='Rules & discovery'}
      <V2Toolbar sticky={false}><b>Rules & discovery</b><V2Badge tone="ok" text="Demo configuration"/></V2Toolbar>
      <div class="v2-setting-grid"><V2Card title="Exact matches"><V2Stack gap="sm"><V2Checkbox label="Detect identical file hashes" checked={true}/><V2Checkbox label="Group exact copies automatically" checked={true}/><V2Button>Save exact-match rules</V2Button></V2Stack></V2Card><V2Card title="Similarity discovery"><V2Stack gap="sm"><V2Field label="Minimum similarity" value="82"/><V2Field label="Maximum candidates per asset" value="20"/><V2Button variant="primary">Save discovery rules</V2Button></V2Stack></V2Card></div>
    {:else}
      <V2Toolbar sticky={false}><V2Badge text="12 resolutions"/>{#snippet actions()}<V2Button>Export</V2Button>{/snippet}</V2Toolbar>
      <V2Stack gap="sm">{#each [['Today 09:42','Group 1','Kept 1 · deleted 1'],['Yesterday 18:10','Group 3','Stacked 3 assets'],['Aug 30 14:22','Group 5','Reviewed · no action']] as row}<V2Card><V2Inline justify="between" wrap={true}><V2Stack gap="xs"><b>{row[1]}</b><span class="v2-small v2-muted">{row[0]}</span></V2Stack><span>{row[2]}</span></V2Inline></V2Card>{/each}</V2Stack>
    {/if}
  </V2Zone>
...
{#if compare}
  <div class="v2-compare-viewer" role="dialog" aria-modal="true">
    <div class="v2-compare-top"><V2Inline gap="sm" wrap={true}><V2Button onclick={()=>compare=false}>✕</V2Button><b>Duplicate comparison</b><V2Badge text={`Group ${group}`}/><V2Badge text={`${activeCount} images`}/></V2Inline><V2Inline gap="sm" wrap={true}><V2Segmented items={['Side by side','Swipe','Transparency','Difference']} active={mode} onselect={(value)=>setMode(value as Mode)} ariaLabel="Comparison mode"/><V2Button onclick={()=>zoom=Math.max(10,zoom/1.25)}>−</V2Button><span class="v2-zoom-readout">{Math.round(zoom)}%</span><V2Button onclick={()=>zoom=Math.min(800,zoom*1.25)}>+</V2Button><V2Button onclick={fit}>Fit</V2Button><V2Button onclick={actual}>1:1</V2Button><V2Button onclick={prev}>← Previous</V2Button><V2Button onclick={next}>Next →</V2Button><V2Button onclick={()=>reference=member}>Set as reference</V2Button><V2Button onclick={()=>window.alert('Mock integrity analysis: current asset is healthy.')}>Analyze integrity</V2Button></V2Inline></div>
    <div class="v2-compare-main"><section class="v2-compare-visual"><div class="v2-compare-stage" class:single={mode!=='Side by side'} onpointerdown={pointerDown} onpointermove={pointerMove} onwheel={wheel}>{#if mode==='Side by side'}<div class="v2-compare-pane" bind:this={compareViewport}><span class="v2-compare-label">Selected image</span><div class="v2-compare-transform" style={`transform:${transform}`}><img src={selectedImage} alt={`Member ${member+1}`}></div></div><div class="v2-compare-pane reference"><span class="v2-compare-label">Reference / keeper candidate</span><div class="v2-compare-transform" style={`transform:${transform}`}><img src={referenceImage} alt={`Reference member ${reference+1}`}></div></div>{:else}<div class={`v2-compare-overlay mode-${mode.toLowerCase().replaceAll(' ','-')}`} bind:this={compareViewport}>{#if mode==='Difference'}<div class="v2-compare-transform" style={`transform:${transform}`}><img src={differenceImage} alt="Generated difference preview"></div><div class="v2-difference-controls"><label><span>Color</span><input type="range" min="0" max="360" bind:value={diffHue}><b>{diffHue}°</b></label><V2Checkbox label="Two colors only" checked={diffBinary} onchange={(checked)=>diffBinary=checked}/>{#if !diffBinary}<label><span>Contrast</span><input type="range" min="50" max="300" bind:value={diffContrast}><b>{diffContrast}%</b></label>{/if}</div>{:else}<div class="v2-compare-layer"><div class="v2-compare-transform" style={`transform:${transform}`}><img src={referenceImage} alt={`Reference member ${reference+1}`}></div></div><div class="v2-compare-layer top" style={mode==='Transparency'?`opacity:${opacity/100}`:`clip-path:inset(0 ${100-split}% 0 0)`}><div class="v2-compare-transform" style={`transform:${transform}`}><img src={selectedImage} alt={`Selected member ${member+1}`}></div></div>{#if mode==='Swipe'}<div class="v2-compare-hover"><span>Split</span><input type="range" min="0" max="100" bind:value={split}><b>{split}%</b></div>{/if}{#if mode==='Transparency'}<div class="v2-compare-hover"><span>Transparency</span><input type="range" min="0" max="100" bind:value={opacity}><b>{opacity}%</b></div>{/if}{/if}<div class="v2-compare-legend"><span>Reference {reference+1}</span><span>Selected {member+1}</span></div></div>{/if}</div><div class="v2-filmstrip">{#each Array.from({length:activeCount},(_,i)=>i) as i}{@const data=comparisonMemberData(group,i)}<button class="v2-thumb" class:active={i===member} class:reference={i===reference} onclick={()=>member=i}><img src={demoCompareImage(group,i)} alt={`Member ${i+1} example`}><small>Member {i+1}</small><small class="v2-muted">{data.size} · {data.similarity}%</small></button>{/each}</div></section><aside class="v2-compare-data"><V2Section title="Quick comparison"><V2Card><V2Stack gap="sm"><V2Inline justify="between"><span>Visual similarity</span><b>{selectedData.similarity}%</b></V2Inline><V2Inline justify="between"><span>File size difference</span><b>{selectedData.sizeNum-referenceData.sizeNum>=0?'+':''}{(selectedData.sizeNum-referenceData.sizeNum).toFixed(1)} MB</b></V2Inline><V2Inline justify="between"><span>Resolution</span><b>{selectedData.dims===referenceData.dims?'Same':'Different'}</b></V2Inline><V2Inline justify="between"><span>Integrity</span><V2Badge tone="ok" text="Healthy"/></V2Inline></V2Stack></V2Card></V2Section><V2Section title="Metadata side by side"><div class="v2-compare-grid"><b>Selected</b><b>Reference</b><span>{selectedData.name}</span><span>{referenceData.name}</span><span>{selectedData.source}</span><span>{referenceData.source}</span><span class:changed={selectedData.size!==referenceData.size}>{selectedData.size}</span><span class:changed={selectedData.size!==referenceData.size}>{referenceData.size}</span><span class:changed={selectedData.dims!==referenceData.dims}>{selectedData.dims}</span><span class:changed={selectedData.dims!==referenceData.dims}>{referenceData.dims}</span><span class:changed={selectedData.taken!==referenceData.taken}>{selectedData.taken}</span><span class:changed={selectedData.taken!==referenceData.taken}>{referenceData.taken}</span><span>{selectedData.codec}</span><span>{referenceData.codec}</span><span>{selectedData.library}</span><span>{referenceData.library}</span><span class:changed={selectedData.uploaded!==referenceData.uploaded}>{selectedData.uploaded}</span><span class:changed={selectedData.uploaded!==referenceData.uploaded}>{referenceData.uploaded}</span></div></V2Section><V2Section title="Decision context"><V2Card><V2Stack gap="sm"><b>Highlighted cells differ.</b><span class="v2-small v2-muted">Use image evidence and metadata together. Similarity alone does not authorize deletion.</span></V2Stack></V2Card></V2Section></aside></div>
