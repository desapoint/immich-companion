<script lang="ts">
  ...
  const sourceIds=$derived((libraryData.state.revision,libraryData.state.assets.slice(0,37).map((asset)=>asset.id)));
...
<V2PageLayout title="Duplicates" description="Review duplicate groups built from the same assets used by the active V2 data source.">
  ...
  {#snippet inspector()}<V2Zone>{#if tab==='Review'}<V2Section title="Batch readiness"><V2Card><V2Stack gap="sm"><b>{selectedGroups.length} selected groups</b><span class="v2-small v2-muted">Decisions reference shared asset IDs; execution updates Assets and Restore through the active data source.</span><V2Inline gap="sm"><V2Badge text={`${decisionCount} planned actions`}/></V2Inline></V2Stack></V2Card></V2Section>{:else if tab==='Rules & discovery'}<V2Section title="Discovery status"><V2Card><span class="v2-small v2-muted">Discovery configuration remains intentionally UI-only; review actions use the active data source.</span></V2Card></V2Section>{/if}</V2Zone>{/snippet}
</V2PageLayout>
