...
<V2PageLayout title="Settings" description="Configure background load limits, duplicate defaults and incremental/global synchronization schedules.">
  ...
  <V2Zone>
    ...
    <div class="v2-setting-grid">
      <V2Card><V2Stack gap="sm"><V2Inline justify="between"><h3 class="v2-section-heading">Interface density</h3><V2Badge tone="ok" text="Local preference"/></V2Inline><span class="v2-small v2-muted">Controls spacing, table row height, card padding and grid thumbnail density across collection interfaces.</span><V2Segmented items={['Standard','Condensed']} active={density === 'standard' ? 'Standard' : 'Condensed'} onselect={(value) => setDensity(value === 'Standard' ? 'standard' : 'condensed')} ariaLabel="Interface density" /></V2Stack></V2Card>
      <V2Card><V2Stack gap="sm"><V2Inline justify="between"><h3 class="v2-section-heading">Background batch load</h3><V2Badge tone="ok" text="Saved"/></V2Inline><V2Field label="Assets per batch · 1–500" type="number" value="100"/><V2Field label="Minimum delay (seconds) · 0–60" type="number" value="1"/><V2Button variant="primary">Save load settings</V2Button></V2Stack></V2Card>
