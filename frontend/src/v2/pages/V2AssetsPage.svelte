...
<V2PageLayout title="Assets" description="Search, browse, select, synchronize and perform guarded actions on assets.">
  ...
  {#snippet context()}<V2Zone label="Context rail"><V2Section title="Search mode"><V2Segmented items={['Simple','Expert']} active={searchMode} onselect={(value)=>searchMode=value as 'Simple'|'Expert'} ariaLabel="Search mode" /></V2Section>{#if searchMode==='Simple'}<V2Section title="Filters"><V2Stack gap="sm"><V2Field label="Filename" placeholder="Filename contains…"/><V2Field label="Media type" options={['Any','Image','Video']}/><V2Field label="Favorite" options={['Any','Favorite','Not favorite']}/><V2Field label="Archived" options={['Any','Archived','Not archived']}/><V2Button>Advanced filters</V2Button></V2Stack></V2Section>{:else}<V2Section title="Expert search">{#snippet actions()}<V2Badge text="Boolean"/>{/snippet}<V2Stack gap="sm"><V2Field label="Saved expert search" options={['Choose saved search…','Favorite images not archived','Family album or Vacation tag','Large landscape images']} onchange={loadSaved}/><V2Card><V2Stack gap="sm"><V2Inline justify="between"><b>Current expression</b><V2Badge text={`${rules.length+groups.reduce((n,g)=>n+g.rules.length,0)} rules · ${groups.length} groups`}/></V2Inline><span class="v2-small v2-muted">{expression}</span><V2Button block={true} onclick={openDrawer}>Edit expression</V2Button></V2Stack></V2Card></V2Stack></V2Section>{/if}<V2Inline gap="sm"><V2Button variant="primary" onclick={()=>summary=searchMode==='Expert'?`Expert search · ${expression}`:'Simple search · submitted filters'}>Search assets</V2Button><V2Button onclick={()=>{summary=`${searchMode} search · cleared and reloaded`;if(searchMode==='Expert'){rules=[];groups=[]}}}>Clear</V2Button></V2Inline></V2Zone>{/snippet}
  ...
  <V2Zone>
    <V2Toolbar label="Primary content">
      ...
      {#snippet actions()}<V2Select inline={true} options={['Taken date ↓']} ariaLabel="Sort assets"/><V2Select inline={true} value={pageSize} options={[{value:'24',label:'24 / batch'},{value:'48',label:'48 / batch'},{value:'96',label:'96 / batch'}]} onchange={(value)=>setPageSize(Number(value))} ariaLabel="Assets per batch"/><V2Segmented items={['Pagination','Infinite']} active={resultMode} onselect={(value)=>setMode(value as 'Pagination'|'Infinite')} ariaLabel="Result loading mode" />{/snippet}
    </V2Toolbar>
