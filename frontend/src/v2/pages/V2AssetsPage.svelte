<script lang="ts">
  ...
  let tab=$state<AssetTab>('Browse'),searchMode=$state<AssetSearchMode>('Simple'),appliedSearchMode=$state<AssetSearchMode>('Simple'),viewer=$state(false),viewerAssetId=$state<string|null>(null),drawer=$state(false),saveSearchOpen=$state(false),selectedSaved=$state(''),sort=$state('takenDate:desc');
