<script lang="ts">
  ...
  import { applyShiftAssetRange,emptyAssetSelection,getAssetSelectionCount,invertAssetSelection,isAllVisibleSelected,isAssetSelected,resolveAssetSelectionPrimary,selectAllMatchingAssets,selectVisibleAssets,toggleAssetSelected,type AssetSelectionState } from '../components/assetSelection';
  ...
  async function runSearch(){if(selectionActive){searchTransitionError='';searchSelectionOpen=true;return}clearSelection();await applySearch()}
  async function continueSearch(keepSelection:boolean){if(searchTransitionBusy)return;searchTransitionBusy=true;searchTransitionError='';try{if(keepSelection&&selection.allMatchingSelected){const selectedIds=await libraryData.assets.materializeSelection(selectionTarget());selection=selectVisibleAssets(selectedIds);reconcileStackPrimary();resetStackReview()}else if(!keepSelection)clearSelection();searchSelectionOpen=false;await applySearch()}catch(error){searchTransitionError=errorMessage(error,keepSelection?'The current selection could not be preserved.':'The search could not be applied.')}finally{searchTransitionBusy=false}}
  ...
  function finishSelectionInteraction(){const changed=interaction.isDragging();interaction.finish();if(changed){reconcileStackPrimary(selection.anchor);resetStackReview();void refreshSelectionCapabilities()}}
  function clearSelection(){capabilityRequests.cancel();removableRelations.cancel();selection=emptyAssetSelection<string>();moreOpen=false;removeRelationDialog=null;selectionCapabilities=emptyCapabilities();selectionError='';stackPrimaryAssetId=null;resetStackReview()}
  function selectVisible(){selection=selectVisibleAssets(ids);stackPrimaryAssetId=ids[0]??null;resetStackReview();void refreshSelectionCapabilities()}
  function selectAllMatching(){selection=selectAllMatchingAssets(ids[0]??null);stackPrimaryAssetId=ids[0]??null;resetStackReview();void refreshSelectionCapabilities()}
  function invertSelection(){selection=invertAssetSelection(selection);reconcileStackPrimary();resetStackReview();void refreshSelectionCapabilities()}
  function toggleViewerSelection(id:string){const wasSelected=isAssetSelected(selection,id);selection=toggleAssetSelected(selection,id);reconcileStackPrimary(wasSelected?null:id);resetStackReview();void refreshSelectionCapabilities()}
  function handleSelectionClick(id:string,event:MouseEvent){const wasSelected=isAssetSelected(selection,id);selection=event.shiftKey?applyShiftAssetRange(selection,ids,id):toggleAssetSelected(selection,id);reconcileStackPrimary(wasSelected?null:id);resetStackReview();void refreshSelectionCapabilities()}
  ...
  async function removeAllTags(){await mutations.run('Remove all tags',(target)=>libraryData.assets.removeTags(target),selectionTarget());moreOpen=false}
  async function removeAllAlbums(){await mutations.run('Remove all albums',(target)=>libraryData.assets.removeFromAlbums(target),selectionTarget());moreOpen=false}
  ...
  async function stackSelected(){if(!selectionCapabilities.canStack||stackPlanning)return;reconcileStackPrimary();if(!stackPrimaryAssetId)return;stackPlanning=true;mutations.clearError();try{stackPlan=await libraryData.assets.planStack(selectionTarget(),stackPrimaryAssetId);stackResolution=null;moreOpen=false}catch(error){mutations.error=errorMessage(error,'The stack action could not be reviewed.')}finally{stackPlanning=false}}
  async function confirmStack(){const initial=stackPlan;if(!initial||mutations.busy)return;if(initial.conflicts.length&&!stackResolution)return;const target=selectionTarget(),primary=initial.primaryAssetId,resolution=stackResolution;const result=await mutations.run('Stack assets',async()=>{const reviewed=initial.conflicts.length?await libraryData.assets.planStack(target,primary,resolution??undefined):initial;return libraryData.assets.executeStack(reviewed.id)},target,{refresh:refreshStackMutation});if(result){stackPlan=null;clearSelection()}}
  async function removeFromStack(){await mutations.run('Remove from stack',(target)=>libraryData.assets.unstack(target),selectionTarget(),{refresh:refreshStackMutation});moreOpen=false}
  async function setPrimary(){if(!singleSelectedId)return;await mutations.run('Set stack primary',()=>libraryData.assets.setStackPrimary(singleSelectedId),{kind:'ids',ids:[singleSelectedId]},{refresh:refreshStackMutation});moreOpen=false}
  async function removeCompleteStack(){if(!singleSelectedId)return;await mutations.run('Remove complete stack',()=>libraryData.assets.removeCompleteStack(singleSelectedId),{kind:'ids',ids:[singleSelectedId]},{refresh:refreshStackMutation});moreOpen=false}
  ...
  function openRemoveRelationDialog(kind:Exclude<RemoveRelationDialog,null>){removeRelationDialog=kind;removeRelationValues=[];moreOpen=false;void removableRelations.load(selectionTarget())}
  async function applyRemoveRelationDialog(){const kind=removeRelationDialog,ids=[...removeRelationValues];if(!kind||!ids.length)return;const result=kind==='album'?await mutations.run('Remove selected albums',(target)=>libraryData.assets.removeFromAlbums(target,ids),selectionTarget()):await mutations.run('Remove selected tags',(target)=>libraryData.assets.removeTags(target,ids),selectionTarget());if(result)removeRelationDialog=null}
...
<V2PageLayout title="Assets" description="Search, browse, select and mutate assets through the active V2 data source.">
  ...
  <V2Zone>{#if tab==='Browse'}
    {#if loadError||selectionError||relations.error||removableRelations.error||savedSearches.error}<V2ErrorState title="Asset operation failed" message={loadError||selectionError||relations.error||removableRelations.error||savedSearches.error} onretry={()=>void retryPageError()}/>{/if}
