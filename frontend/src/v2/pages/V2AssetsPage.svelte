<script lang="ts">
  ...
  import { assetExpressionText,assetRulesInGroups,assetSearchCounts,buildAssetCriteria,cloneAssetGroups,emptyAssetAdvanced,emptyAssetSimple,hydrateAssetGroups,splitAssetIds,type AssetGroup,type AssetRule,type AssetSearchMode,type AssetSimpleSnapshot } from '../state/assetSearch';
  ...
  function selectedOptionValues(kind:'album'|'tag'){
    ...
    const expert=[...rules,...assetRulesInGroups(groups),...draftRules,...assetRulesInGroups(draftGroups)].filter((rule)=>rule.field===field).flatMap((rule)=>splitAssetIds(rule.value));
