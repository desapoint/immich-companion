<script lang="ts">
  ...
  const appliedSearchSummary=$derived.by(()=>{
    if(appliedSearchMode==='Expert'){
      const ruleCount=appliedRules.length+appliedGroups.reduce((count,group)=>count+group.rules.length,0);
      if(!ruleCount)return'Expert search · No rules';
      return`Expert search · ${assetExpressionText(appliedRules,appliedGroups,appliedLogic,appliedNegated)}`;
    }
    const parts:string[]=[];
    if(appliedSimple.filename)parts.push(`Filename: ${appliedSimple.filename}`);
    if(appliedSimple.mediaType)parts.push(appliedSimple.mediaType);
    if(appliedSimple.favorite)parts.push(appliedSimple.favorite);
    if(appliedSimple.archived)parts.push(appliedSimple.archived);
    const advanced=appliedSimple.advanced;
    const albumIds=splitAssetIds(advanced.albumIds),tagIds=splitAssetIds(advanced.tagIds);
    if(albumIds.length)parts.push(`${albumIds.length} album${albumIds.length===1?'':'s'}`);
    if(tagIds.length)parts.push(`${tagIds.length} tag${tagIds.length===1?'':'s'}`);
    if(advanced.noAlbum)parts.push('No album');
    if(advanced.noTag)parts.push('No tag');
    if(advanced.takenAfter||advanced.takenBefore)parts.push('Date range');
    if(advanced.minWidth||advanced.maxWidth||advanced.minHeight||advanced.maxHeight)parts.push('Dimensions');
    if(advanced.minAspectRatio||advanced.maxAspectRatio)parts.push('Aspect ratio');
    return parts.length?`Simple search · ${parts.join(' · ')}`:'Simple search · No filters';
  });
  ...
  async function runSearch(){appliedSearchMode=searchMode;if(searchMode==='Simple')appliedSimple={filename,mediaType,favorite,archived,advanced:{...simpleAdvanced}};else{appliedRules=rules.map((rule)=>({...rule}));appliedGroups=groups.map((group)=>({...group,rules:group.rules.map((rule)=>({...rule}))}));appliedLogic=logic;appliedNegated=negated}collection.reset();clearSelection();await refreshSearch(true)}
  ...
  async function trashSelected(){if(!selectionActive)return;await mutations.run('Move to trash',(target)=>libraryData.assets.trash(target),selectionTarget());clearSelection()}
  ...
  async function retryPageError(){mutations.clearError();relations.error='';savedSearches.error='';await Promise.all([refreshSearch(true),searchAlbumOptions(relations.albumQuery),searchTagOptions(relations.tagQuery),savedSearches.refresh()])}
...
<svelte:window onclick={handleWindowClick} onpointermove={interaction.move} onpointerup={interaction.finish} onpointercancel={interaction.cancel} onkeydown={(event)=>{if(event.key==='Escape'){if(interaction.isDragging())interaction.cancel();else if(relationDialog)relationDialog=null;else if(moreOpen)moreOpen=false;else if(drawer)drawer=false;else if(viewer)viewer=false;else if(selectionActive)clearSelection()}}}/>
