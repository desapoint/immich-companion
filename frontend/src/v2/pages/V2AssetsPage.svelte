<script lang="ts">
  ...
  import { assetExpressionText,assetRulesInGroups,assetSearchCounts,buildAssetCriteria,cloneAssetGroups,emptyAssetAdvanced,emptyAssetSimple,hydrateAssetGroups,simpleAssetSearchToExpert,splitAssetIds,type AssetGroup,type AssetRule,type AssetSearchMode,type AssetSimpleSnapshot } from '../state/assetSearch';
  ...
  function convertSimpleToExpert(){
    const converted=simpleAssetSearchToExpert({filename,mediaType,favorite,archived,advanced:{...simpleAdvanced}},()=>++seq);
    rules=converted.rules;groups=converted.groups;logic=converted.logic;negated=converted.negated;searchMode='Expert';openDrawer();
  }
