<script lang="ts">
  ...
  async function setSelectedFavorite(){if(!selectionActive)return;const next=favoriteActionLabel==='Favorite',target=await exactSelectionTarget();await mutations.run(next?'Favorite':'Unfavorite',(value)=>libraryData.assets.setFavorite(value,next),target);moreOpen=false}
  async function setSelectedArchived(){if(!selectionActive)return;const next=archiveActionLabel==='Archive',target=await exactSelectionTarget();await mutations.run(next?'Archive':'Unarchive',(value)=>libraryData.assets.setArchived(value,next),target);moreOpen=false}
  async function trashSelected(){if(!selectionActive||mutations.busy)return;const target=await exactSelectionTarget(),result=await mutations.run('Move to trash',(value)=>libraryData.assets.trash(value),target);trashConfirmOpen=false;if(result)clearSelection()}
  async function syncSelected(){const target=await exactSelectionTarget();await mutations.run('Sync',(value)=>libraryData.assets.sync(value),target);moreOpen=false}
  async function removeAllTags(){const target=await exactSelectionTarget();await mutations.run('Remove all tags',(value)=>libraryData.assets.removeTags(value),target);moreOpen=false}
  async function removeAllAlbums(){const target=await exactSelectionTarget();await mutations.run('Remove all albums',(value)=>libraryData.assets.removeFromAlbums(value),target);moreOpen=false}
