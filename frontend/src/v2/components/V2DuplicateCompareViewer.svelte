<script lang="ts">
  ...
  let { open, groupTitle, groupKind, groupSimilarity=null, similarityEngine=null, similarityModelVersion=null, similarityFeatureVersion=null, similarityComparisonVersion=null, assetIds=[], similarities={}, member=$bindable(0), reference=$bindable(0), decisions=$bindable<Record<string,DuplicateDecision>>({}), decisionOptions=[], stackLabel='Stack', stackPrimary=false, disabled=false, ondecisionchange, ondecisionclear, onstackprimary, onreferencechange, onclose }: { open:boolean; groupTitle:string; groupKind:string; groupSimilarity?:number|null; similarityEngine?:string|null; similarityModelVersion?:string|null; similarityFeatureVersion?:number|null; similarityComparisonVersion?:number|null; assetIds?:string[]; similarities?:Record<string,number|null>; member?:number; reference?:number; decisions?:Record<string,DuplicateDecision>; decisionOptions?:DuplicateDecision[]; stackLabel?:string; stackPrimary?:boolean; disabled?:boolean; ondecisionchange?:(assetId:string,decision:DuplicateDecision)=>void; ondecisionclear?:(assetId:string)=>void; onstackprimary?:(assetId:string)=>void; onreferencechange?:(assetId:string)=>Promise<void>; onclose:()=>void }=$props();
...
<style>
  ...
  .v2-compare-engine{max-width:18rem;text-align:right;overflow-wrap:anywhere}
