<script lang="ts">
  ...
  let { open, groupTitle, groupKind, assetIds=[], similarities={}, member=$bindable(0), reference=$bindable(0), decisions=$bindable<Record<string,DuplicateDecision>>({}), decisionOptions=[], stackLabel='Stack', stackPrimary=false, disabled=false, ondecisionchange, ondecisionclear, onstackprimary, onreferencechange, onclose }: { open:boolean; groupTitle:string; groupKind:string; assetIds?:string[]; similarities?:Record<string,number>; member?:number; reference?:number; decisions?:Record<string,DuplicateDecision>; decisionOptions?:DuplicateDecision[]; stackLabel?:string; stackPrimary?:boolean; disabled?:boolean; ondecisionchange?:(assetId:string,decision:DuplicateDecision)=>void; ondecisionclear?:(assetId:string)=>void; onstackprimary?:(assetId:string)=>void; onreferencechange?:(assetId:string)=>Promise<void>; onclose:()=>void }=$props();
  ...
  let mode=$state<ComparisonMode>('Side by side'),split=$state(50),opacity=$state(50),diffHue=$state(190),diffContrast=$state(180),diffBinary=$state(true),diffTolerance=$state(8),assets=$state<AssetRecord[]>([]),memberData=$state<ComparisonMemberData[]>([]);
...
<style>
  ...
  .v2-compare-footer-context{display:flex;align-items:center;gap:var(--v2-space-2);min-width:0;max-width:100%}
  .v2-compare-footer-label{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .v2-compare-footer-decisions{flex:0 1 32rem;width:min(32rem,100%);min-width:min(22rem,100%)}
