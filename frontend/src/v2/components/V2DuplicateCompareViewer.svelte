<script lang="ts">
  ...
  let { open, groupTitle, groupKind, assetIds=[], similarities={}, member=$bindable(0), reference=$bindable(0), decisions=$bindable<Record<string,DuplicateDecision>>({}), decisionOptions=[], stackLabel='Stack', stackPrimary=false, disabled=false, ondecisionchange, onstackprimary, onreferencechange, onclose }: { open:boolean; groupTitle:string; groupKind:string; assetIds?:string[]; similarities?:Record<string,number>; member?:number; reference?:number; decisions?:Record<string,DuplicateDecision>; decisionOptions?:DuplicateDecision[]; stackLabel?:string; stackPrimary?:boolean; disabled?:boolean; ondecisionchange?:(assetId:string,decision:DuplicateDecision)=>void; onstackprimary?:(assetId:string)=>void; onreferencechange?:(assetId:string)=>Promise<void>; onclose:()=>void }=$props();
