<script lang="ts">
  ...
  let mode=$state<ComparisonMode>('Side by side'),split=$state(50),opacity=$state(50),diffHue=$state(190),diffContrast=$state(180),diffBinary=$state(true),assets=$state<AssetRecord[]>([]),memberData=$state<ComparisonMemberData[]>([]),decisionOptions=$state<DuplicateDecision[]>([]);
  ...
  function visualSource(asset:AssetRecord|undefined,full=true){if(!asset)return'';if(asset.asset_type==='VIDEO')return videoPlaceholder;return full?libraryData.media.fullSize(asset):libraryData.media.thumbnail(asset)}
  const activeCount=$derived(assetIds.length),selectedAsset=$derived(assets.find((asset)=>asset.id===assetIds[member])),referenceAsset=$derived(assets.find((asset)=>asset.id===assetIds[reference])),selectedData=$derived(memberData[member]??emptyData),referenceData=$derived(memberData[reference]??emptyData),selectedImage=$derived(visualSource(selectedAsset)),referenceImage=$derived(visualSource(referenceAsset)),differenceImage=$derived(selectedAsset&&referenceAsset&&selectedAsset.asset_type!=='VIDEO'&&referenceAsset.asset_type!=='VIDEO'?libraryData.media.difference(selectedAsset,referenceAsset,{hue:diffHue,contrast:diffContrast,binary:diffBinary}):videoPlaceholder),decisionKey=$derived(assetIds[member]??'');
