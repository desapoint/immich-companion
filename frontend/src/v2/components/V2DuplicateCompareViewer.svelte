<script lang="ts">
  ...
  import { comparisonMemberData, formatSimilarityPercent, type ComparisonMemberData } from '../data/duplicateMember';
  ...
  let { open, groupTitle, groupKind, groupSimilarity=null, assetIds=[], similarities={}, similarityEvidence={}, member=$bindable(0), reference=$bindable(0), decisions=$bindable<Record<string,DuplicateDecision>>({}), decisionOptions=[], stackLabel='Stack', stackPrimary=false, disabled=false, ondecisionchange, ondecisionclear, onstackprimary, onreferencechange, onrevalidate, onclose }: { open:boolean; groupTitle:string; groupKind:string; groupSimilarity?:number|null; assetIds?:string[]; similarities?:Record<string,number|null>; similarityEvidence?:Record<string,DuplicateSimilarityEvidence|null>; member?:number; reference?:number; decisions?:Record<string,DuplicateDecision>; decisionOptions?:DuplicateDecision[]; stackLabel?:string; stackPrimary?:boolean; disabled?:boolean; ondecisionchange?:(assetId:string,decision:DuplicateDecision)=>void; ondecisionclear?:(assetId:string)=>void; onstackprimary?:(assetId:string)=>void; onreferencechange?:(assetId:string)=>Promise<void>; onrevalidate?:(assetId:string)=>Promise<void>; onclose:()=>void }=$props();
  ...
  function showMember(index:number){const target=comparisonTargetId(assetIds,assetIds[reference]??'',assetIds[index]??'');member=Math.max(0,assetIds.indexOf(target))}
  function stepMember(direction:'previous'|'next'){if(!activeCount)return;const target=stepComparisonTargetId(assetIds,assetIds[reference]??'',assetIds[member]??'',direction);member=Math.max(0,assetIds.indexOf(target))}
  ...
  async function setReference(){if(!selectedAsset)return;if(onreferencechange){await onreferencechange(selectedAsset.id);return}reference=member;showMember(reference)}
  async function revalidate(){const assetId=assetIds[reference];if(assetId&&onrevalidate)await onrevalidate(assetId)}
