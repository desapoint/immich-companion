<script lang="ts">
  ...
  import { comparisonMemberData, formatSimilarityPercent, usesBoundedValidation, type ComparisonMemberData } from '../data/duplicateMember';
  ...
  const boundedValidation = $derived(
    selectedEvidence?.detailSource === 'preview'
    || usesBoundedValidation(selectedAsset)
    || usesBoundedValidation(referenceAsset)
  );
...
<V2ViewerShell {open} title="Duplicate comparison" kind="compare" {onclose}>
  ...
  <div class="v2-compare-main"><section class="v2-compare-visual">
    ...
    <div class="v2-filmstrip">{#each assetIds as assetId,index (assetId)}{@const asset=assetById.get(assetId)}{@const data=memberData[index]??emptyData}<button class="v2-thumb" class:active={index===member} class:reference={index===reference} onclick={()=>showMember(index)}>{#if asset}<span class="v2-thumb-media"><V2LazyAssetMedia cacheKey={`duplicate-compare-thumbnail:${asset.id}`} resolve={()=>libraryData.media.thumbnail(asset)} alt={data.name}/></span>{/if}<small>{data.name}</small><small class="v2-muted">{data.size} · {data.similarity}{usesBoundedValidation(asset)?' · bounded':''}</small></button>{/each}</div>
  </section><aside class="v2-compare-data"><V2Section title="Quick comparison"><V2Card><V2Stack gap="sm"><V2Inline justify="between"><span>Match type</span><b>{matchLabel}</b></V2Inline><V2Inline justify="between"><span>Group similarity</span><b>{groupSimilarityLabel}</b></V2Inline><V2Inline justify="between"><span>Similarity to reference</span><b>{selectedData.similarity}</b></V2Inline>{#if boundedValidation}<V2Inline justify="between"><span>Validation evidence</span><V2Badge tone="warn" text="Bounded"/></V2Inline><span class="v2-small v2-muted">At least one side cannot be fully decoded within Companion's 64-megapixel safety limit, or its original/full-size detail evidence was unavailable. Similarity therefore uses bounded Immich-generated visual evidence. This is safer for very large images, but very small pixel-level differences can be less visible than with full-resolution validation.</span>{/if}{#if selectedEvidence}<V2Inline justify="between"><span>Structure <small class="v2-muted">· 65% weight</small></span><b>{percent(selectedEvidence.structuralPercent)}</b></V2Inline><V2Inline justify="between"><span>Perceptual hash <small class="v2-muted">· 25% weight</small></span><b>{percent(selectedEvidence.perceptualPercent)}</b></V2Inline><V2Inline justify="between"><span>Color <small class="v2-muted">· 10% weight</small></span><b>{percent(selectedEvidence.colorPercent)}</b></V2Inline>{#if selectedEvidence.detailChangedPercent!==null&&selectedEvidence.detailChangedPercent!==undefined}<V2Inline justify="between"><span>Detail changed area <small class="v2-muted">· sampled</small></span><b>{percent(selectedEvidence.detailChangedPercent)}</b></V2Inline><V2Inline justify="between"><span>Detail evidence</span><V2Badge tone={selectedEvidence.detailSource==='preview'?'warn':'ok'} text={selectedEvidence.detailSource==='original'?'Original':selectedEvidence.detailSource==='transcoded'?'Full-size conversion':'Bounded preview'}/></V2Inline><span class="v2-small v2-muted">The final similarity includes this candidate detail check. Difference shows displayed-pixel changes; Local changes shows the validator grid.</span>{/if}{:else}<V2Inline justify="between"><span>Visual score details</span><b>Not calculated</b></V2Inline>{/if}<V2Inline justify="between"><span>Peak local change <small class="v2-muted">· validator</small></span><b>{localDiagnosticsLoading?'Calculating…':localDiagnostics?.available?optionalPercent(localDiagnostics.localizedChangedPercent):'Unavailable'}</b></V2Inline><V2Inline justify="between"><span>Coherent changed area <small class="v2-muted">· validator</small></span><b>{localDiagnosticsLoading?'Calculating…':localDiagnostics?.available?optionalPercent(localDiagnostics.coherentChangedPercent):'Unavailable'}</b></V2Inline><V2Inline justify="between"><span>Largest changed region <small class="v2-muted">· validator</small></span><b>{localDiagnosticsLoading?'Calculating…':localDiagnostics?.available?optionalPercent(localDiagnostics.largestChangedRegionPercent):'Unavailable'}</b></V2Inline><V2Inline justify="between"><span>Substantial regions <small class="v2-muted">· validator</small></span><b>{localDiagnosticsLoading?'Calculating…':localDiagnostics?.available?(localDiagnostics.substantialRegionCount??'—'):'Unavailable'}</b></V2Inline><span class="v2-small v2-muted">The detail validator uses this local grid and coherent-region topology when scoring candidate similarity. The Local changes view explains that evidence; it is not a separate mismatch rule.</span><V2Inline justify="between"><span>File size difference</span><b>{sizeDifferenceLabel}</b></V2Inline><V2Inline justify="between"><span>Resolution</span><b>{selectedData.dims===referenceData.dims?'Same':'Different'}</b></V2Inline>{#if sameSourceCollection}<V2Inline justify="between"><span>{folderScopeLabel}</span><V2Badge tone={!foldersComparable?'warn':sameFolder?'ok':'warn'} text={!foldersComparable?'Unavailable':sameFolder?'Same':'Different'}/></V2Inline>{/if}<V2Inline justify="between"><span>Difference source</span><V2Badge tone="ok" text="Layered displayed pixels"/></V2Inline></V2Stack></V2Card></V2Section><V2Section title="Metadata side by side"><div class="v2-compare-grid" role="table" aria-label="Selected and reference asset metadata"><div class="v2-compare-grid-heading v2-compare-grid-property" role="columnheader">Property</div><div class="v2-compare-grid-heading" role="columnheader"><span>Selected</span><small title={selectedData.name}>{selectedData.name}</small></div><div class="v2-compare-grid-heading" role="columnheader"><span>Reference</span><small title={referenceData.name}>{referenceData.name}</small></div>{#each metadataRows as row (row.label)}<div class="v2-compare-grid-label" role="rowheader">{row.label}</div><div class:changed={row.changed} class="v2-compare-grid-value" role="cell">{row.selected}</div><div class:changed={row.changed} class="v2-compare-grid-value" role="cell">{row.reference}</div>{/each}</div></V2Section></aside></div>
