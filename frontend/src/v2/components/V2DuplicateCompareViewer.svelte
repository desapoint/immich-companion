<script lang="ts">
  ...
  import { comparisonMemberData } from '../data/duplicateMember';
  ...
  const selectedAsset=$derived((libraryData.state.revision,libraryData.assets.getById(assetIds[member]??'')));
  const referenceAsset=$derived((libraryData.state.revision,libraryData.assets.getById(assetIds[reference]??'')));
  const selectedData=$derived((libraryData.state.revision,comparisonMemberData(assetIds[member]??'',group,member)));
  const referenceData=$derived((libraryData.state.revision,comparisonMemberData(assetIds[reference]??'',group,reference)));
  const selectedImage=$derived(selectedAsset?libraryData.media.fullSize(selectedAsset):'');
  const referenceImage=$derived(referenceAsset?libraryData.media.fullSize(referenceAsset):'');
  const differenceImage=$derived(selectedAsset&&referenceAsset?libraryData.media.difference(selectedAsset,referenceAsset,{hue:diffHue,contrast:diffContrast,binary:diffBinary}):'');
...
<V2ViewerShell {open} title="Duplicate comparison" kind="compare" {onclose}>
  ...
  <div class="v2-compare-main"><section class="v2-compare-visual"><V2ImageComparison selectedSrc={selectedImage} referenceSrc={referenceImage} differenceSrc={differenceImage} selectedLabel={selectedData.name} referenceLabel={referenceData.name} bind:mode bind:opacity bind:split bind:diffHue bind:diffContrast bind:diffBinary/>
    ...
    <V2Section title="Decision context"><V2Card><V2Stack gap="sm"><b>Decisions target asset IDs.</b><span class="v2-small v2-muted">All duplicate metadata except similarity is derived from the active asset data source.</span></V2Stack></V2Card></V2Section>
  </aside></div>
