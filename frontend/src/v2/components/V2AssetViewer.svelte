<script lang="ts">
  ...
  const currentIndex=$derived(currentId?assetIds.indexOf(currentId):-1),media=$derived(asset?libraryData.media.view(asset):null),mediaSrc=$derived(media?.url??''),posterSrc=$derived(media?.posterUrl??(asset?libraryData.media.thumbnail(asset).url:'')),isVideo=$derived(asset?.asset_type==='VIDEO'),needsDecodedImage=$derived(media?.delivery==='decoded'),needsVideoProxy=$derived(media?.delivery==='transcoded'),canPrevious=$derived(currentIndex>0),canNext=$derived(currentIndex>=0&&currentIndex<assetIds.length-1),positionLabel=$derived(currentIndex>=0?`${currentIndex+1} / ${assetIds.length}`:'—'),sizeLabel=$derived(asset?.file_size_bytes?`${(asset.file_size_bytes/1_048_576).toFixed(1)} MB`:'Unknown size');
