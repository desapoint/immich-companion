<script lang="ts">
  ...
  const positionLabel=$derived(navigation.position!==null?`${navigation.position} / ${navigation.total}`:fallbackIndex>=0?`${fallbackIndex+1} / ${assetIds.length}`:'—'),sizeLabel=$derived(formatFileSize(asset?.file_size_bytes));
  ...
  <div class="v2-viewer-stage">
    ...
    <aside class="v2-viewer-info">
      ...
      {#if asset}
        ...
        <V2Section title="Relationships"><V2Card><div class="viewer-relations"><div class="viewer-relation"><b>Albums</b><div class="viewer-pills">{#each asset.albums as album (album.id)}<V2Badge text={album.name} title={`Search assets in ${album.name}`} onclick={()=>filterRelationship('album',album.id)}/>{:else}<span class="v2-small v2-muted">No albums</span>{/each}</div></div><div class="viewer-relation"><b>Tags</b><div class="viewer-pills">{#each asset.tags as tag (tag.id)}<V2Badge text={tag.name} title={`Search assets tagged ${tag.name}`} onclick={()=>filterRelationship('tag',tag.id)}/>{:else}<span class="v2-small v2-muted">No tags</span>{/each}</div></div><div class="viewer-status-grid"><div><b>Favorite</b><span class="v2-small">{asset.is_favorite?'Yes':'No'}</span></div><div><b>Archived</b><span class="v2-small">{asset.is_archived?'Yes':'No'}</span></div></div>{#if asset.stack}<div class="viewer-relation"><b>Stack</b><span class="v2-small">{asset.stack.assetCount} assets · {isStackPrimary?'Primary asset':'Stack member'}</span></div>{/if}</div></V2Card></V2Section>
