<script lang="ts">
  ...
  async function loadCurrent(id:string){
    const request=++loadRequest;loading=true;assetError='';mediaError='';navigationError='';navigationLoading=true;actionError='';relationDialog=null;
    try{
  ...
  async function runAction(label:string,action:(id:string)=>Promise<unknown>,reloadAfter=true){
    if(!asset||actionBusy)return;
    const id=asset.id;actionBusy=true;actionError='';
    try{await action(id);if(reloadAfter)await reload()}
    catch(error){actionError=errorMessage(error,`${label} could not be completed.`)}
  ...
  async function trash(){
    ...
    try{
      await libraryData.assets.trash(target(id));
      if(fallback&&await libraryData.assets.getById(fallback))currentId=fallback;else onclose();
    }catch(error){actionError=errorMessage(error,'Move to trash could not be completed.')}
    finally{actionBusy=false}
  }
  ...
  function openRelationDialog(kind:RelationDialog){
    ...
    relationDialog=kind;relationAlbum='';relationTags=[];actionError='';
  ...
  async function applyRelationDialog(){
    ...
    const kind=relationDialog;
    actionBusy=true;actionError='';
    try{
      if(kind==='album'&&relationAlbum)await libraryData.assets.addToAlbum(target(asset.id),relationAlbum);
      if(kind==='tags'&&relationTags.length)await libraryData.assets.addTags(target(asset.id),relationTags);
      relationDialog=null;
      await reload();
    }catch(error){actionError=errorMessage(error,kind==='album'?'Add to album could not be completed.':'Add tags could not be completed.')}
    finally{actionBusy=false}
  }
...
{#if relationDialog}
  <V2AssetRelationModal
    ...
    oncreated={async(kind,option)=>{if(!asset)return;if(kind==='album')await libraryData.assets.addToAlbum(target(asset.id),option.value);else await libraryData.assets.addTags(target(asset.id),[option.value]);await reload()}}
