...
<span bind:this={host} class="v2-lazy-asset-media" data-loaded={Boolean(url) && !failed} aria-busy={!url && !failed}>
  {#if failed}
    <span class="v2-lazy-asset-placeholder v2-lazy-asset-failed" aria-label="Thumbnail unavailable"><ImageOff size={25} aria-hidden="true"/><small>Preview unavailable</small></span>
  {:else if !url}
...
<style>
  .v2-lazy-asset-media{display:block;width:100%;height:100%;min-width:0;min-height:0;background:#0d141d;overflow:hidden}
  .v2-lazy-asset-media img,.v2-lazy-asset-media video{display:block;width:100%;height:100%;object-fit:cover}
  .v2-lazy-asset-placeholder{width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.35rem;color:var(--v2-muted);background:linear-gradient(135deg,#101923,#0c1219);opacity:.72}
  .v2-lazy-asset-placeholder small{font-size:.72rem}
</style>
