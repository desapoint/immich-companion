<script lang="ts">
  ...
  import type { StackConflict, StackResolution, StackResolutionMap } from '../data/contracts';
  import {
  ...
  let {
    ...
    busy?: boolean;
    onresolutionchange: (reviewId: string, resolution: StackResolutionMap) => void;
    onconfirm: () => void;
    onclose: () => void;
  } = $props();

  const options: Array<{ value: StackResolution; label: string; description: string }> = [
    { value: 'move_selected', label: 'Move selected', description: 'Move only highlighted members into the new stack. The old stack keeps unselected members when possible.' },
    { value: 'keep_existing', label: 'Keep existing', description: 'Leave this existing stack untouched and exclude its selected members from the new stack.' },
    { value: 'include_existing', label: 'Include whole stack', description: 'Bring every member of this existing stack into the new stack.' },
  ];

  const conflictCount = $derived(reviews.reduce((count, review) => count + review.plan.conflicts.length, 0));
  const sharedIds = $derived(sharedConflictIds(reviews));
  const allComplete = $derived(reviews.every((review) => stackReviewComplete(review) && !hasSharedMutation(review)));
  ...
  function optionAllowed(review: StackConflictReviewTarget, conflict: StackConflict, choice: StackResolution): boolean {
    ...
    if (choice === 'keep_existing' && conflict.selectedAssetIds.includes(review.plan.primaryAssetId)) return false;
  ...
  function change(review: StackConflictReviewTarget, conflict: StackConflict, value: string): void {
    ...
    onresolutionchange(review.id, withConflictResolution(review.plan, review.resolution, conflict.stackId, choice));
  }
  ...
  function applyAll(choice: StackResolution): void {
    for (const review of reviews) {
      ...
      if (changed) onresolutionchange(review.id, next);
    }
  }
  ...
  function resolutionWarning(review: StackConflictReviewTarget): string | null {
    ...
    const current = choices(review);
    const primaryConflict = review.plan.conflicts.find((conflict) => conflict.selectedAssetIds.includes(review.plan.primaryAssetId));
...
<V2ConfirmDialog
  ...
  message={conflictCount === 1 ? 'One existing stack overlaps the stack you are creating. Review how it should be reconciled.' : `${conflictCount.toLocaleString()} existing stack conflicts overlap the stacks you are creating. Resolve each one before continuing.`}
          ...
          {#each review.plan.conflicts as conflict, index (conflict.stackId)}
            <article class="v2-stack-conflict" data-shared={sharedIds.has(conflict.stackId) || undefined}>
              ...
              <div class="v2-stack-thumbnails" aria-label={`Members of existing stack ${index + 1}`}>
                {#each conflict.memberAssetIds as assetId (assetId)}
                  <div
                    class="v2-stack-thumb"
                    class:selected={conflict.selectedAssetIds.includes(assetId)}
                    class:primary={conflict.primaryAssetId === assetId}
                    title={`${conflict.selectedAssetIds.includes(assetId) ? 'Selected for new stack' : 'Existing stack member'}${conflict.primaryAssetId === assetId ? ' · current primary' : ''}`}
                  >
                    ...
                    {#if conflict.selectedAssetIds.includes(assetId)}<span class="v2-stack-thumb-selected"><Check size={10} aria-hidden="true"/> Selected</span>{/if}
                  </div>
                {/each}
              </div>
...
<style>
  ...
  .v2-stack-description{margin:0;color:var(--v2-muted);font-size:12px;line-height:1.35}
