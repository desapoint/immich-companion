<script lang="ts">
  ...
  const shortcuts:KeyboardShortcut[]=[
    ...
    {keys:'0',description:'Fit image'},
    {keys:'1',description:'Actual pixel size'},
  ];
  let currentId=$state<string|null>(assetId),asset=$state<TrashAssetRecord|undefined>(),media=$state<MediaResource|null>(null),navigation=$state<ViewerNavigationWindow>(emptyNavigation());
  ...
  const fallbackIndex=$derived(currentId?assetIds.indexOf(currentId):-1),mediaSrc=$derived(media?.url??''),posterSrc=$derived(media?.posterUrl??(asset?libraryData.media.thumbnail(asset).url:'')),isVideo=$derived(asset?.type==='VIDEO');
...
<V2ViewerShell {open} title="Restore Viewer" {onclose}>
  {#snippet header()}<V2Inline gap="sm"><V2Button onclick={onclose}>✕</V2Button><b>Restore Viewer</b><V2Badge text={navigationLoading?'Locating…':positionLabel}/>{#if media?.delivery==='transcoded'}<V2Badge text="Transcoded playback"/>{:else if media?.delivery==='decoded'}<V2Badge text="Decoded preview"/>{/if}</V2Inline><V2Inline gap="sm">{#if !isVideo}<V2ZoomControl value={camera.zoom} onzoomout={()=>camera.setZoom(camera.zoom/1.25)} onzoomin={()=>camera.setZoom(camera.zoom*1.25)}/><V2Button onclick={()=>camera.fit()} title="Fit image">Fit</V2Button><V2Button onclick={()=>camera.actual()} title="Actual pixel size">1:1</V2Button>{/if}<V2KeyboardShortcuts {shortcuts}/></V2Inline>{/snippet}
