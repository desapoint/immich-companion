<script lang="ts">
  ...
  const asset=$derived((libraryData.state.revision,libraryData.assets.listTrash().find((item)=>item.id===currentId)));
  ...
  async function restore(){if(!asset)return;const fallback=assetIds[currentIndex+1]??assetIds[currentIndex-1]??null;await libraryData.assets.restore([asset.id]);if(fallback&&libraryData.assets.listTrash().some((item)=>item.id===fallback))currentId=fallback;else onclose()}
</script>
...
<V2ViewerShell {open} title="Restore Viewer" {onclose}>
  ...
  <div class="v2-viewer-stage"><div class="v2-image-stage">{#if asset}<V2ImageViewport src={imageSrc} alt={asset.original_file_name} controller={camera}/>{/if}</div><aside class="v2-viewer-info">{#if asset}<V2Section title="Details"><V2Card><b>{asset.original_file_name}</b><p class="v2-small v2-muted">{asset.width??'—'} × {asset.height??'—'} · {asset.original_mime_type??'Unknown type'}</p></V2Card></V2Section><V2Section title="Metadata"><V2Card><span class="v2-small">Taken {new Date(asset.taken_at).toLocaleString()}<br>Modified {new Date(asset.file_modified_at).toLocaleString()}<br>{asset.restore_path??'No restore path'}</span></V2Card></V2Section><V2Section title="Restore boundary"><V2Card><span class="v2-small">Restoring this item lets the active data source restore its related album, tag and stack state when available.</span></V2Card></V2Section>{:else}<V2Section title="Asset"><V2Card><span class="v2-muted">This item is no longer in trash.</span></V2Card></V2Section>{/if}</aside></div>
