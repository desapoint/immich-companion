<script lang="ts">
  ...
  function emptyFilters(): SimpleAdvancedFilters {
    return {
      albumIds: '', tagIds: '', noAlbum: false, noTag: false, takenAfter: '', takenBefore: '',
  ...
  function countActive(value: SimpleAdvancedFilters): number {
    return splitIds(value.albumIds).length + splitIds(value.tagIds).length + Number(value.noAlbum) + Number(value.noTag)
      + [value.takenAfter,value.takenBefore,value.minWidth,value.maxWidth,value.minHeight,value.maxHeight,value.minAspectRatio,value.maxAspectRatio].filter((entry) => entry.trim()).length;
  }
...
<V2Button block active={activeCount > 0} onclick={show}>Advanced{activeCount > 0 ? ` · ${activeCount}` : ''}</V2Button>
...
{#if open}
  ...
  <aside class="v2-drawer" aria-label="Advanced simple search filters">
    ...
    <div class="v2-drawer-body"><V2Stack gap="md">
      ...
      <V2Section title="Aspect ratio"><div class="v2-advanced-grid"><V2Field label="Minimum aspect ratio" value={draft.minAspectRatio} placeholder="16:9 or 1.778" onchange={(value)=>update('minAspectRatio',value)}/><V2Field label="Maximum aspect ratio" value={draft.maxAspectRatio} placeholder="4:3 or 1.333" onchange={(value)=>update('maxAspectRatio',value)}/></div></V2Section>
    </V2Stack></div>
...
<style>
  .v2-advanced-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.7rem; }
  @media (max-width: 42rem) { .v2-advanced-grid { grid-template-columns:1fr; } }
</style>
