...
{#if rebuildOpen}
  ...
  >
    {#snippet detail()}
      <V2Stack gap="sm">
        <span>Active similarity scan/index work is cancelled. Search fingerprints, detailed validation, pair scores, completed similarity scans, unavailable markers, and the derived duplicate projection are invalidated.</span>
        <span>Review decisions and resolution history are not deleted. A fresh full-library similarity scan is durably queued in the same server transaction using your saved discovery threshold, validation mode, and candidate limit.</span>
      </V2Stack>
    {/snippet}
  </V2ConfirmDialog>
{/if}
