<script lang="ts">
  ...
  function addRule(){update({rules:[...group.rules,{id:nextId(),field:'filename',op:'contains',value:''}]})}
  ...
  function addGroup(){update({groups:[...(group.groups??[]),{id:nextId(),logic:'AND',negated:false,rules:[{id:nextId(),field:'tag',op:'is',value:''}],groups:[]}]})}
