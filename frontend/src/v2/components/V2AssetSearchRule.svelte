...
<div class="v2-expert-rule">
  ...
  <SelectField id={`${idPrefix}-op-${rule.id}`} value={rule.op} options={assetOperatorOptionsForField(rule.field)} onchange={(value)=>update({op:value})}/>
  {#if rule.field==='album'||rule.field==='tag'}
