<script lang="ts">
  ...
  function add(event?:MouseEvent|KeyboardEvent){event?.preventDefault();event?.stopPropagation();if(disabled||!onadd)return;const request:SelectAddRequest={query:searchQuery.trim(),shiftKey:Boolean(event?.shiftKey)};open=false;searchQuery='';activeIndex=-1;onadd(request)}
  ...
  function handleSearchKey(event:KeyboardEvent){if(event.key==='ArrowDown'){event.preventDefault();activeIndex=firstEnabled();void tick().then(()=>list?.querySelector<HTMLButtonElement>(`[data-index="${activeIndex}"]`)?.focus())}else if(event.key==='ArrowUp'){event.preventDefault();activeIndex=visibleOptions.length;move(-1)}else if(event.key==='Enter'){const first=visibleOptions[firstEnabled()];if(first){event.preventDefault();choose(first)}else if(onadd){event.preventDefault();add(event)}}else if(event.key==='Escape'){event.preventDefault();open=false;searchQuery='';void tick().then(()=>trigger?.focus())}}
