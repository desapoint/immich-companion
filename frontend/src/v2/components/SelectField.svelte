<script lang="ts">
  ...
  function normalizeStringOption(option: string): NormalizedSelectOption[] {
    ...
    const [, optionLabel, arrow] = directional;
    ...
    return ([currentDirection, alternateDirection] as SortDirection[]).map((direction): NormalizedSelectOption => ({
      value: valueFor(direction), label: optionLabel, subtitle: '', disabled: false, direction,
    }));
  }
  ...
  function normalizeOption(option: SelectOption): NormalizedSelectOption[] {
    ...
    return [{ value: option.value, label: option.label, subtitle: option.subtitle ?? '', disabled: option.disabled ?? false, direction: option.direction }];
  }
  ...
  function firstEnabled(): number { return visibleOptions.findIndex((option) => !option.disabled); }
  ...
  function getNativeScrollbarWidth(): number {
    ...
    probe.style.position = 'fixed'; probe.style.left = '-10000px'; probe.style.top = '-10000px';
    probe.style.width = '100px'; probe.style.height = '100px'; probe.style.overflow = 'scroll'; probe.style.pointerEvents = 'none';
  ...
  function measureIntrinsicPopupWidth(): void {
    ...
    intrinsicPopupWidth = Math.max(Math.ceil(measuredPopup + scrollbarAllowance), searchable ? 240 + scrollbarAllowance : 0);
  }
  ...
  function updateMultiTriggerText(): void {
    if (!multiple) return;
    if (selectedOptions.length === 0) { multiTriggerText = placeholder; return; }
    const available = Math.max(40, (trigger?.clientWidth ?? 0) - 74);
    const labels = selectedOptions.map((option) => option.label);
    const all = labels.join(', ');
    if (measureText(all) <= available) { multiTriggerText = all; return; }
    for (let shown = labels.length - 1; shown >= 1; shown -= 1) {
      const candidate = `${labels.slice(0, shown).join(', ')} +${labels.length - shown}`;
      if (measureText(candidate) <= available) { multiTriggerText = candidate; return; }
    }
    multiTriggerText = `${labels.length} selected`;
  }

  function positionPopup(): void {
    if (!open || !trigger) return;
    const margin = 10, gap = 5, desiredMaxHeight = 304, minimumUsefulHeight = 144;
    ...
    const leftAligned = rect.left, rightAligned = rect.right - popupWidth;
    if (leftAligned + popupWidth <= window.innerWidth - margin) { popupLeft = Math.max(margin, leftAligned); popupAlignment = 'left'; }
    else if (rightAligned >= margin) { popupLeft = rightAligned; popupAlignment = 'right'; }
    else { popupLeft = Math.min(Math.max(leftAligned, margin), window.innerWidth - popupWidth - margin); popupAlignment = 'viewport'; }
    popupTop = popupPlacement === 'down' ? rect.bottom + gap : Math.max(margin, rect.top - gap - Math.min(optionsPopup?.scrollHeight ?? popupMaxHeight, popupMaxHeight));
  }
  ...
  function show(): void {
    ...
    open = true;
    void tick().then(() => {
      positionPopup(); requestAnimationFrame(positionPopup);
  ...
  function choose(option: NormalizedSelectOption | undefined): void {
    ...
    if (multiple) {
      ...
      values = normalized.filter((candidate) => next.has(candidate.value)).map((candidate) => candidate.value);
      onvalueschange?.(values);
      void tick().then(updateMultiTriggerText);
      return;
    }
    value = option.value;
  ...
  function handleTriggerKey(event: KeyboardEvent): void {
    if (event.key === 'ArrowDown' || event.key === 'ArrowUp' || event.key === 'Enter' || event.key === ' ') { event.preventDefault(); show(); }
  }

  function handleSearchKey(event: KeyboardEvent): void {
    if (event.key === 'ArrowDown') { event.preventDefault(); activeIndex = firstEnabled(); void tick().then(() => list?.querySelector<HTMLButtonElement>(`[data-index="${activeIndex}"]`)?.focus()); }
    else if (event.key === 'ArrowUp') { event.preventDefault(); activeIndex = visibleOptions.length; move(-1); }
    else if (event.key === 'Enter') { const first = visibleOptions[firstEnabled()]; if (first) { event.preventDefault(); choose(first); } }
  ...
  function handleOptionKey(event: KeyboardEvent, index: number): void {
    ...
    else if (event.key === 'ArrowUp') { event.preventDefault(); if (searchable && index === firstEnabled()) searchInput?.focus(); else move(-1); }
  ...
  $effect(() => {
    ...
    const selectedSignature = multiple ? values.join('\u0001') : String(value);
    void tick().then(() => { void optionSignature; void currentSearchable; void selectedSignature; measureIntrinsicPopupWidth(); updateMultiTriggerText(); if (open) positionPopup(); });
  });

  $effect(() => {
    if (!open) return;
    const reposition = () => { positionPopup(); updateMultiTriggerText(); };
    ...
    <button bind:this={trigger} {id} class="v2-select-trigger" data-placeholder={isEmpty || undefined} type="button" {disabled} aria-haspopup="listbox" aria-expanded={open} aria-controls={`${id}-options`} onclick={() => (open ? open = false : show())} onkeydown={handleTriggerKey}>
      {#if width === 'content' && !multiple}
        <span class="v2-select-trigger-copy" style="display:grid;overflow:visible">
          ...
          {#if allowEmpty}<span aria-hidden="true" style="grid-area:1 / 1;visibility:hidden;display:inline-flex;align-items:center;gap:6px;width:max-content;white-space:nowrap"><span>{placeholder}</span></span>{/if}
          {#each normalized as option (`trigger-sizer-${option.value}`)}
            <span aria-hidden="true" style="grid-area:1 / 1;visibility:hidden;display:inline-flex;align-items:center;gap:6px;width:max-content;white-space:nowrap"><span>{option.label}</span>{#if option.direction === 'asc'}<ArrowUp class="v2-select-direction-icon" size={14} aria-hidden="true" />{/if}{#if option.direction === 'desc'}<ArrowDown class="v2-select-direction-icon" size={14} aria-hidden="true" />{/if}</span>
          {/each}
        </span>
      {:else}
        <span class="v2-select-trigger-copy">
          <span>{multiple ? multiTriggerText : isEmpty ? placeholder : selected?.label ?? placeholder}</span>
          {#if !multiple && !isEmpty && selected?.direction === 'asc'}<ArrowUp class="v2-select-direction-icon" size={14} aria-hidden="true" />{/if}
          {#if !multiple && !isEmpty && selected?.direction === 'desc'}<ArrowDown class="v2-select-direction-icon" size={14} aria-hidden="true" />{/if}
        </span>
      {/if}
      <span class="v2-select-chevron" aria-hidden="true"></span>
    </button>
    {#if (multiple && values.length > 0) || (!multiple && allowEmpty && !isEmpty)}
      <button class="v2-select-clear" type="button" disabled={disabled} aria-label={`Clear ${label || (multiple ? 'selections' : 'selection')}`} onclick={clear}>×</button>
    {/if}
  </div>

  <div bind:this={widthProbe} aria-hidden="true" style="position:fixed;left:-10000px;top:-10000px;visibility:hidden;pointer-events:none;width:max-content;max-width:none">
    <div data-select-width-popup class="v2-select-options" data-searchable={searchable || undefined} style="position:static;left:auto;top:auto;width:max-content;min-width:0;max-width:none;max-height:none;overflow:visible;visibility:hidden">
