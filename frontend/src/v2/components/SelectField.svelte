<script lang="ts">
  ...
  function positionPopup(){if(!open||!trigger)return;const rect=trigger.getBoundingClientRect(),layout=floatingFieldLayout({anchor:rect,viewportWidth:window.innerWidth,viewportHeight:window.innerHeight,preferredWidth:Math.min(544,Math.max(rect.width,intrinsicPopupWidth)),preferredHeight:Math.min(304,optionsPopup?.scrollHeight||304),minimumUsefulHeight:144,minimumHeight:96});popupPlacement=layout.placement;popupAlignment=layout.alignment;popupTop=layout.top;popupLeft=layout.left;popupWidth=layout.width;popupMaxHeight=layout.maxHeight}
...
<style>
  .v2-select-add{width:100%;display:flex;align-items:center;gap:7px;border:0;border-top:1px solid var(--v2-line);border-radius:0;background:transparent;color:#9bb9e2;padding:7px 8px;cursor:pointer;text-align:left;font:inherit}
