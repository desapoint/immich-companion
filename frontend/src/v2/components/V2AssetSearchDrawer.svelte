<script lang="ts">
  ...
  import { assetExpressionText,assetFieldSelectOptions,assetOperatorOptionsForField,type AssetGroup,type AssetRule } from '../state/assetSearch';
  ...
  function changeField(rule:AssetRule,value:string){rule.field=value;rule.op=assetOperatorOptionsForField(value)[0]?.value??'is';rule.value=''}
  ...
  <div class="v2-drawer-body">
    <V2Section title="Expression structure"><V2Stack gap="md">
      <V2Card><V2Stack gap="sm"><V2Inline justify="between" wrap><V2Inline gap="sm"><V2Badge text="Root group"/><V2Segmented items={['AND','OR']} active={logic} onselect={(value)=>logic=value as 'AND'|'OR'} ariaLabel="Root group logic"/><V2Checkbox label="NOT group" checked={negated} onchange={(checked)=>negated=checked}/></V2Inline></V2Inline>
        {#each rules as rule (rule.id)}<div class="v2-expert-rule"><SelectField id={`expert-root-field-${rule.id}`} value={rule.field} options={assetFieldSelectOptions} onchange={(value)=>changeField(rule,value)}/><SelectField id={`expert-root-op-${rule.id}`} value={rule.op} options={assetOperatorOptionsForField(rule.field)} onchange={(value)=>rule.op=value}/>{#if rule.field==='album'||rule.field==='tag'}<SelectField id={`expert-root-value-${rule.id}`} value={rule.value} searchable allowEmpty placeholder={`Choose ${rule.field}…`} options={rule.field==='album'?albumOptions:tagOptions} loading={rule.field==='album'?albumLoading:tagLoading} hasMore={rule.field==='album'?albumHasMore:tagHasMore} onsearchchange={rule.field==='album'?onalbumsearch:ontagsearch} onloadmore={rule.field==='album'?onalbumloadmore:ontagloadmore} onchange={(value)=>rule.value=value}/>{:else}<input bind:value={rule.value} placeholder="Value…" onkeydown={handleSearchKeydown}>{/if}<V2Button onclick={()=>removeRule(rule.id)}>✕</V2Button></div>{/each}
      ...
      {#each groups as group (group.id)}<V2Card><V2Stack gap="sm"><V2Inline justify="between"><V2Inline gap="sm"><V2Badge text="Nested group"/><V2Segmented items={['AND','OR']} active={group.logic} onselect={(value)=>group.logic=value as 'AND'|'OR'} ariaLabel="Nested group logic"/><V2Checkbox label="NOT group" checked={group.negated} onchange={(checked)=>group.negated=checked}/></V2Inline><V2Button onclick={()=>groups=groups.filter((item)=>item.id!==group.id)}>Remove group</V2Button></V2Inline>
        {#each group.rules as rule (rule.id)}<div class="v2-expert-rule"><SelectField id={`expert-group-${group.id}-field-${rule.id}`} value={rule.field} options={assetFieldSelectOptions} onchange={(value)=>changeField(rule,value)}/><SelectField id={`expert-group-${group.id}-op-${rule.id}`} value={rule.op} options={assetOperatorOptionsForField(rule.field)} onchange={(value)=>rule.op=value}/>{#if rule.field==='album'||rule.field==='tag'}<SelectField id={`expert-group-${group.id}-value-${rule.id}`} value={rule.value} searchable allowEmpty placeholder={`Choose ${rule.field}…`} options={rule.field==='album'?albumOptions:tagOptions} loading={rule.field==='album'?albumLoading:tagLoading} hasMore={rule.field==='album'?albumHasMore:tagHasMore} onsearchchange={rule.field==='album'?onalbumsearch:ontagsearch} onloadmore={rule.field==='album'?onalbumloadmore:ontagloadmore} onchange={(value)=>rule.value=value}/>{:else}<input bind:value={rule.value} onkeydown={handleSearchKeydown}>{/if}<V2Button onclick={()=>removeRule(rule.id,group)}>✕</V2Button></div>{/each}<V2Button onclick={()=>addRule(group)}>+ Rule</V2Button>
      </V2Stack></V2Card>{/each}
    </V2Stack></V2Section>
