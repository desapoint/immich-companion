...
<div class="v2-cron-field" data-invalid={!valid || undefined}>
  ...
  <div class="v2-cron-presets" aria-label={`${label} quick schedules`}>{#each quickPresets as preset (preset.cron)}<V2Button active={value===preset.cron} onclick={()=>applyPreset(preset.cron)}>{preset.label}</V2Button>{/each}</div>
