...
import V2ColorSwatch from './ColorSwatch.svelte';

describe('ColorField', () => {
  it('renders a normalized current color as an accessible closed field', () => {
    const { body } = render(ColorField, { props: { id: 'tag-color-1', label: 'Color', value: '#9a78ff' } });
  ...
  it('renders the nullable and disabled states without exposing the popup', () => {
    const { body } = render(ColorField, { props: { id: 'tag-color-empty', label: 'Color', value: null, disabled: true } });
