<script lang="ts">
  ...
  import type { ToastAction, ToastController } from '../../../app/state/toasts.svelte';

  let { controller }: { controller: ToastController } = $props();
  function runAction(id: number, action: ToastAction): void {
...
<div class="v2-toast-viewport" data-position={controller.position} aria-label="Notifications">
  {#each controller.toasts as toast (toast.id)}
    <Toast {toast} ondismiss={() => controller.dismiss(toast.id)} onaction={() => toast.action && runAction(toast.id, toast.action)}/>
  {/each}
</div>
