...
import { ToastController } from '../../../app/state/toasts.svelte';
...
describe('ToastViewport', () => {
  it('renders multiple notifications in chronological DOM order at the configured anchor', () => {
    const controller = new ToastController('top-left');
    ...
    const { body } = render(ToastViewport, { props: { controller } });
  ...
  it('uses alert semantics for errors', () => {
    ...
    expect(render(ToastViewport, { props: { controller } }).body).toContain('role="alert"');
    controller.destroy();
  });
});
