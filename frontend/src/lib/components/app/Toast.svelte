<script lang="ts">
  ...
  import V2Notice from '../ui/Notice.svelte';
  import type { Toast } from '../../../app/state/toasts.svelte';

  let { toast, ondismiss, onaction }: { toast: Toast; ondismiss: () => void; onaction: () => void } = $props();
</script>
