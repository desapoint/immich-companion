...
import {
  ...
  type SelectionWorkspaceMembership,
  type SelectionWorkspaceRecord,
  type SelectionWorkspaceRepository,
} from './selectionWorkspace.svelte';

type TestWorkspace = SelectionWorkspaceRecord;

const active = (revision = 0, selectedCount = 0): TestWorkspace => ({
...
function fixture() {
  ...
  const repository = {
    ...
    } as unknown as SelectionWorkspaceRepository<{ query: string }>;
...
describe('V2 database-backed asset selection workspace', () => {
  ...
  it('updates visible state immediately and debounces member writes', async () => {
    ...
    const controller = new SelectionWorkspaceController(repository, storage, 'immich-companion:v2:tag-selection');
  ...
  it('flushes pending member writes before producing a bulk-action target', async () => {
    vi.useFakeTimers();
    const { repository, storage } = fixture();
    const controller = new SelectionWorkspaceController(repository, storage, 'immich-companion:v2:asset-selection');
  ...
  it('keeps interaction made while database membership is loading', async () => {
    let resolveMembership!: (value: SelectionWorkspaceMembership<TestWorkspace> | null) => void;
    const { repository, storage } = fixture();
    repository.selectionMembership = vi.fn(() => new Promise<SelectionWorkspaceMembership<TestWorkspace> | null>((resolve) => { resolveMembership = resolve; }));
    storage.setItem('immich-companion:v2:asset-selection', 'selection-1');
    const controller = new SelectionWorkspaceController(repository, storage, 'immich-companion:v2:asset-selection');
  ...
  it('does not resurrect a workspace cleared during an in-flight write', async () => {
    let resolveCreate!: (value: TestWorkspace) => void;
    const { repository, storage } = fixture();
    repository.createSelection = vi.fn(() => new Promise<TestWorkspace>((resolve) => { resolveCreate = resolve; }));
    const controller = new SelectionWorkspaceController(repository, storage, 'immich-companion:v2:asset-selection');
    controller.setMembers(['a'], true);
